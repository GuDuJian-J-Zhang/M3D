...
const string NoteFactory::Serializer_XML_Element_ThreeDGestureNote =
		"ThreeDGestureNote";
...
const string NoteFactory::Serializer_XML_Element_ThreeDGestureNoteLine =
		"ThreeDGestureNoteLine";
...
const string NoteFactory::Serializer_XML_Element_CompositeTexts =
		"CompositeTexts";
...
const string NoteFactory::Serializer_XML_Attribute_IsParallelScreen =
		"IsParallelScreen";
...
void NoteFactory::GetMeshSet(BoundingBox& box, vector<Vector3>&triangleSet,
		SceneManager* scene) {
...
void NoteFactory::GetInsectShape(void* data, SceneNode* node) {
	...
	if (model) {
		...
		if (lineBox.IsInside(box) != OUTSIDE) {
			...
			for (int i = 0; i < bodiesSize; i++) {
				GetInsectBody(lineDatas, bodies[i], lineBox);
			}
			...
			insectMeshSetInfo->meshSet->insert(
					insectMeshSetInfo->meshSet->end(), lineDatas.begin(),
					lineDatas.end());
		}
	}
}
void NoteFactory::GetInsectBody(vector<Vector3>& lineDatas, Body * body,
		BoundingBox& lineBox) {
	if (body) {
...
//		if (insectMeshSetInfo->box->IsInside(box) != OUTSIDE)
		{
			for (int i = 0; i < facesSize; i++) {
				GetInsectFace(lineDatas, faces[i], lineBox);
			}
		}
	}

}
void NoteFactory::GetInsectFace(vector<Vector3>& lineDatas, Face * face,
		BoundingBox& lineBox) {
	if (face) {
...
void NoteFactory::GetInsectMesh(vector<Vector3>& lineDatas, Mesh * mesh,
		BoundingBox& lineBox) {
	if (mesh) {
		...
		if (pSet->IsUseIndex()) {
			vector<unsigned short>* indexArray = pSet->GetIndexArray();
			for (vector<unsigned short>::iterator it = indexArray->begin()
					+ dataOffset;
					it != indexArray->begin() + dataOffset + dataLength;
					it = it + 3) {
				Vector3 points[4];
			...
			for (vector<Vector3>::iterator it = positionArray->begin()
					+ dataOffset;
					it != positionArray->begin() + dataOffset + dataLength;
					it = it + 3) {
				Vector3 points[4];
				points[0] = (*it);
				points[1] = (*(it + 1));
...
string NoteFactory::ThreeDGestureNoteToXMLElement(SceneManager* scene,
	...
	//创建根节点
	XMLElement *pThreeDGestureNote = pDoc.NewElement(
			Serializer_XML_Element_ThreeDGestureNote.c_str());
	pDoc.LinkEndChild(pThreeDGestureNote);
	pThreeDGestureNote->SetAttribute(Serializer_XML_Attribute_ID.c_str(),
			threeDGesturesNote->GetID());
...
//		int line3DSetIndexSize = line3DIndex.size();
		...
		for (int i = 0; i < allSize; i = i + 1) {
			...
			XMLElement * p3DGestureNoteLine = pDoc.NewElement(
					Serializer_XML_Element_ThreeDGestureNoteLine.c_str());
			...
			p3DGestureNoteLine->SetAttribute(
					Serializer_XML_Attribute_Color.c_str(), colorStr.c_str());
			string allPoisitionStr = "";
			vector<Vector3>& originalProjectPnts =
					threeDGesturesNote->GetOriginalProjectPns().at(i);
	...
	{
		XMLElement * pAttributes = pDoc.NewElement(
				Serializer_XML_Element_Attributes.c_str());
		pThreeDGestureNote->LinkEndChild(pAttributes);
		for (map<string, string>::iterator it =
				threeDGesturesNote->GetPropertyMap().begin();
				it != threeDGesturesNote->GetPropertyMap().end(); it++) {
			XMLElement * pAttribute = pDoc.NewElement(
					Serializer_XML_Element_Attribute.c_str());
			pAttributes->LinkEndChild(pAttribute);
			pAttribute->SetAttribute(Serializer_XML_Attribute_Key.c_str(),
					it->first.c_str());
			pAttribute->SetAttribute(Serializer_XML_Attribute_Value.c_str(),
					it->second.c_str());
		}
	}
...
Note* NoteFactory::CreateThreeDGestureNoteFromXMLElement(SceneManager* scene,
		const string& xmlElement) {
	...
	XMLElement* pThreeDGestureNote = doc.FirstChildElement(
			Serializer_XML_Element_ThreeDGestureNote.c_str());
	if (pThreeDGestureNote) {
		int id = pThreeDGestureNote->IntAttribute(
				Serializer_XML_Attribute_ID.c_str());
	...
	XMLElement * p3DGestureNoteLine = pThreeDGestureNote->FirstChildElement(
			Serializer_XML_Element_ThreeDGestureNoteLine.c_str());

	while (p3DGestureNoteLine) {
		float width = p3DGestureNoteLine->FloatAttribute(
				Serializer_XML_Attribute_Width.c_str());
		threeDGesturesNote->SetOriginalProjectWidths(width); //设置宽度
		string colorStr = p3DGestureNoteLine->Attribute(
				Serializer_XML_Attribute_Color.c_str());
		...
		threeDGesturesNote->SetOriginalProjectColors(lineColor); //设置颜色
		...
		for (int i = 0; i < positionSize - 2; i = i + 3) {
			...
			IntVector2 screenPnt =
					scene->GetCamera()->GetViewPort().WorldToScreenPoint(
							startPoint);
			Vector3 tempVec3(screenPnt.m_x, screenPnt.m_y, 0.0f);
			originScreenPnts.push_back(tempVec3);
			originPosition.push_back(startPoint);
		}
		threeDGesturesNote->SetOriginalProjectPns(originPosition); //将投影点加入note
		...
		for (int i = 0; i < finalPoints.size(); i++) {
			for (int j = 0; j < finalPoints[i].size() - 1; j++) {
				Line3D * line = new Line3D(finalPoints[i][j],
						finalPoints[i][j + 1]);
...
//		threeDGesturesNote->SetLine3DIndex(line3dindexSize,line3dindexSize+positionSize/6-1);
		p3DGestureNoteLine = p3DGestureNoteLine->NextSiblingElement(
				Serializer_XML_Element_ThreeDGestureNoteLine.c_str());
	}

	map<string, string> attributeList;
	XMLElement* xmlAttributes = pThreeDGestureNote->FirstChildElement(
			Serializer_XML_Element_Attributes.c_str());
	if (xmlAttributes) {
		XMLElement* xmlAttribute = xmlAttributes->FirstChildElement(
				Serializer_XML_Element_Attribute.c_str());
		while (xmlAttribute) {
			string key = xmlAttribute->Attribute(
					Serializer_XML_Attribute_Key.c_str());
			string value = xmlAttribute->Attribute(
					Serializer_XML_Attribute_Value.c_str());
...
void NoteFactory::InsectPoint(SceneManager * scene,
		vector<Vector3> & originScreenPnts, vector<Vector3> & origin3DPnts,
		vector<vector<Vector3> > &finalPoints) {
...
void NoteFactory::SplitPolyLine(Gesture3DInfo & originalInfo,
		Gesture3DInfo& currentInfo, SceneManager * scene) {
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = 0; j < pointCount; j++) {
			...
			bool intersect1 = scene->GetPickPoint(screenPnt, insectPoint1,
					true);
			if (!intersect1) {
				isNeedCreate = true;
			} else {
...
Note* NoteFactory::CreateThreeDGestureNote(Gesture3DInfo& gesture3DInfos,
		SceneManager * scene) {
...
Note* NoteFactory::CreateCommonNote(CommonNoteInfo& commonNoteInfo,
		SceneManager * scene) {
	...
	if (createImagePath.length() > 0) {
		Vector2 imageBoardSize(commonNoteInfo.m_imageSize.m_x,
				commonNoteInfo.m_imageSize.m_y);
...
Note* NoteFactory::CreateVoiceNote(Vector2& screenPnt, const string& text,
...
Note* NoteFactory::CreateTextNote(int firtShapId, Vector2& screenPnt,
		const string& text, int type, SceneManager* scene,
...
Note*
NoteFactory::CreateSequenceNumberNote(int firtShapId, Vector2& screenPnt,
		const string& text, SceneManager* scene, Properties* properties) {
	Note* note = NULL;
	note = CreatePntSequenceNumberNote(firtShapId, screenPnt, text, scene,
			properties);
	if (note) {
		AddNoteToScene(scene, note);
	}
...
bool NoteFactory::UpdateNoteImagePos(Note* note, Vector2& screenPnt,
		SceneManager* scene) {
	bool ret = false;
	Vector3 tempPos;
...
//				if(imageBoard->GetName() == "TextsImage")
				{
					...
					Ray cameraRay =
							scene->GetCamera()->GetViewPort().GetScreenRay(
					...
					if (note->GetType() == SHAPE_TEXT_NOTE) {
						...
						SequenceNumberNote * tempNote =
								(SequenceNumberNote*) note;
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			if (line) {
				if (line->GetName() == "TextImageLeader"
						|| line->GetName() == "SequenceNumberImageLeader") {
					scene->Lock();
...
bool NoteFactory::UpdateNoteTextValue(Note* note, string text,
		SceneManager * scene) {
	bool ret = false;
	if (note != NULL && scene != NULL) {
		vector<ImageBoard*>& imageboards = note->GetImageBoards();
		for (int i = 0; i < imageboards.size(); i++) {
			...
			if (imageBoard) {
				...
				if (note->GetType() == SHAPE_SEQUENCE_NUMBER_NOTE) {
					...
					imageBoardNew =
							MeasureDisplayHelper::CreateSequenceNumberImage(
									scene, temptext, pos);
					//保存更新内容
					...
					imageBoardNew = MeasureDisplayHelper::createNoteTextsImageN(
							scene, textVector, pos);
...
Note* NoteFactory::CreateTextNoteFromXMLElement(SceneManager* scene,
		const string& xmlElement) {
	...
	XMLElement* xmlNote = doc.FirstChildElement(
			Serializer_XML_Element_TextNote.c_str());
	...
	//textNote->SetID(id);
	bool isParallelScreen = xmlNote->BoolAttribute(
			Serializer_XML_Attribute_IsParallelScreen.c_str());
	...
	bool visible = xmlNote->BoolAttribute(
			Serializer_XML_Attribute_Visible.c_str());
	textNote->SetVisible(visible);

	XMLElement* xmlLeaders = xmlNote->FirstChildElement(
			Serializer_XML_Element_Leaders.c_str());
	//LOGI("xmlLeaders");
	if (xmlLeaders) {
		XMLElement* xmlLeader = xmlLeaders->FirstChildElement(
				Serializer_XML_Element_Leader.c_str());
		if (xmlLeader) {
			//LOGI("xmlLeader");
			XMLElement* xmlPolyline = xmlLeader->FirstChildElement(
					Serializer_XML_Element_Polyline.c_str());
			if (xmlPolyline) {
				//LOGI("xmlPolyline");
				XMLElement* xmlPoint = xmlPolyline->FirstChildElement(
						Serializer_XML_Element_Point.c_str());
				Vector3 temVec[2];
				int i = 0;
				while (xmlPoint) {
					temVec[i].m_x = xmlPoint->FloatAttribute(
							Serializer_XML_Attribute_X.c_str());
					temVec[i].m_y = xmlPoint->FloatAttribute(
							Serializer_XML_Attribute_Y.c_str());
					temVec[i].m_z = xmlPoint->FloatAttribute(
							Serializer_XML_Attribute_Z.c_str());
	...
	XMLElement* xmlCompositeTexts = xmlNote->FirstChildElement(
			Serializer_XML_Element_CompositeTexts.c_str());

	if (xmlCompositeTexts) {
		//LOGI("Begin xmlCompositeTexts");
		XMLElement* compositeText = xmlCompositeTexts->FirstChildElement(
				Serializer_XML_Element_CompositeText.c_str());

		if (compositeText) {
			//LOGI("Begin compositeText");
			XMLElement* texts = compositeText->FirstChildElement(
					Serializer_XML_Element_Texts.c_str());
			if (texts) {
				//LOGI("Begin Texts");
				XMLElement* xmlText = texts->FirstChildElement(
						Serializer_XML_Element_Text.c_str());

				string value = xmlText->Attribute(
						Serializer_XML_Attribute_Value.c_str());
				contentStr = value;
				...
				if (xmlText) {
					XMLElement* xmlTextStyle = xmlText->FirstChildElement(
							Serializer_XML_Element_TextStyle.c_str());

					if (xmlTextStyle) {
						XMLElement* xmlCharHeight =
								xmlTextStyle->FirstChildElement(
						...
						if (xmlCharHeight) {
							...
							float fontSize = StringHelper::StringToFloat(
									fontTextTemp);
							if (fontSize < 1.0)
								fontSize = 15.0f;
							fSize = fontSize;
					...
					XMLElement* xmlLocaltion = xmlText->FirstChildElement(
							Serializer_XML_Element_Location.c_str());
	...
	//解析Attributes元素及子元素
	map<string, string> attributeList;
	XMLElement* xmlAttributes = xmlNote->FirstChildElement(
			Serializer_XML_Element_Attributes.c_str());
	if (xmlAttributes) {
		XMLElement* xmlAttribute = xmlAttributes->FirstChildElement(
				Serializer_XML_Element_Attribute.c_str());
	...
	ImageBoard * imageBord = MeasureDisplayHelper::createNoteTextsImageN(scene,
			textsArray, endPnt);
	LOGE("ImageBoard pointer %p", imageBord);
...
Note* NoteFactory::CreateTextNoteFromJSON(SceneManager* scene,
...
string NoteFactory::TextNoteToXMLElement(SceneManager* scene,
		TextNote* textNote) {
	...
	//创建根节点
	XMLElement *pTextNote = pDoc.NewElement(
			Serializer_XML_Element_TextNote.c_str());
	pDoc.LinkEndChild(pTextNote);
	pTextNote->SetAttribute(Serializer_XML_Attribute_ID.c_str(),
			textNote->GetID());
	pTextNote->SetAttribute(Serializer_XML_Attribute_IsParallelScreen.c_str(),
			textNote->IsParallelScreen());
	pTextNote->SetAttribute(Serializer_XML_Attribute_IsFix.c_str(),
			textNote->IsFix());
	pTextNote->SetAttribute(Serializer_XML_Attribute_Visible.c_str(),
			textNote->IsVisible());
	{
		//创建Color节点
		XMLElement * pColor = pDoc.NewElement(
				Serializer_XML_Element_Color.c_str());
	...
	{
		//Leaders节点
		XMLElement * pLeaders = pDoc.NewElement(
				Serializer_XML_Element_Leaders.c_str());
...
//			{
			XMLElement * pLeader = pDoc.NewElement(
					Serializer_XML_Element_Leader.c_str());
			...
			{
				//创建Terminator
				XMLElement * pTerminator = pDoc.NewElement(
						Serializer_XML_Element_Terminator.c_str());
				pLeader->LinkEndChild(pTerminator);
				pTerminator->SetAttribute(Serializer_XML_Attribute_Type.c_str(),
						4); //TODO:Type
				pTerminator->SetAttribute(
						Serializer_XML_Attribute_Width.c_str(), 3); //TODO:
				pTerminator->SetAttribute(
						Serializer_XML_Attribute_Height.c_str(), 1); //TODO:
				{
					//创建Location
					XMLElement * pLocation = pDoc.NewElement(
							Serializer_XML_Element_Location.c_str());
					pTerminator->LinkEndChild(pLocation);
					pLocation->SetAttribute(Serializer_XML_Attribute_X.c_str(),
							0.0f);
					pLocation->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
							0.0f);
					pLocation->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
							0.0f);

					//创建Direction
					XMLElement * pDirection = pDoc.NewElement(
							Serializer_XML_Element_Direction.c_str());
					pTerminator->LinkEndChild(pDirection);
					pDirection->SetAttribute(Serializer_XML_Attribute_X.c_str(),
							-0.7); //TODO
					pDirection->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
							0.7); //TODO
					pDirection->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
							0.0); //TODO
				}

				//创建Polyline
				XMLElement * pPolyline = pDoc.NewElement(
						Serializer_XML_Element_Polyline.c_str());
				pLeader->LinkEndChild(pPolyline);
				pPolyline->SetAttribute(Serializer_XML_Attribute_ID.c_str(), 0); //TODO:
				pPolyline->SetAttribute(Serializer_XML_Attribute_Type.c_str(),
						1); //TODO:
				pPolyline->SetAttribute(Serializer_XML_Attribute_Min.c_str(),
						0); //TODO:
				pPolyline->SetAttribute(Serializer_XML_Attribute_Max.c_str(),
						0); //TODO:
				{
					//Line3D *line  = new Line3D(textNote->GetNotePos(),textNote->GetTextsPos());
					//创建Point
					{
						XMLElement * pPoint = pDoc.NewElement(
								Serializer_XML_Element_Point.c_str());
						pPolyline->LinkEndChild(pPoint);
						pPoint->SetAttribute(Serializer_XML_Attribute_X.c_str(),
								textNote->GetNotePos().m_x); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
								textNote->GetNotePos().m_y); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
								textNote->GetNotePos().m_z); //TODO
					}
					...
					//创建Point
					{
						...
						pPolyline->LinkEndChild(pPoint);
						pPoint->SetAttribute(Serializer_XML_Attribute_X.c_str(),
								textNote->GetTextsPos().m_x); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
								textNote->GetTextsPos().m_y); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
								textNote->GetTextsPos().m_z); //TODO
					}
...
//			}
		...
		{
			//ExtendLines
			XMLElement * pExtendLines = pDoc.NewElement(
					Serializer_XML_Element_ExtendLines.c_str());
			pTextNote->LinkEndChild(pExtendLines);
		}

		{
			//CompositeTexts
			XMLElement * pCompositeTexts = pDoc.NewElement(
					Serializer_XML_Element_CompositeTexts.c_str());
			pTextNote->LinkEndChild(pCompositeTexts);
			if (textNote->m_ComTexts.size() > 0) {
				//CompositeText
				XMLElement * pCompositeText = pDoc.NewElement(
						Serializer_XML_Element_CompositeText.c_str());
				pCompositeTexts->LinkEndChild(pCompositeText);
				{
					//Texts
					XMLElement * pTexts = pDoc.NewElement(
							Serializer_XML_Element_Texts.c_str());
					pCompositeText->LinkEndChild(pTexts);
					{
						//Text
						XMLElement * pText = pDoc.NewElement(
								Serializer_XML_Element_Text.c_str());
						...
						{
							//TextStyle
							XMLElement * pTextStyle = pDoc.NewElement(
									Serializer_XML_Element_TextStyle.c_str());
							pText->LinkEndChild(pTextStyle);
							{
								//CharHeight
								{
									XMLElement * pCharHeight =
											pDoc.NewElement(
													Serializer_XML_Element_CharHeight.c_str());
									pTextStyle->LinkEndChild(pCharHeight);
									float width = 10.0f, height = 10.0f;
									textNote->m_ComTexts.at(0)->GetCText(0)->GetCharWidthHeight(
											width, height);
									tinyxml2::XMLText * pContent =
											pDoc.NewText(
								...
								//Font
								{
									XMLElement * pFont =
											pDoc.NewElement(
													Serializer_XML_Element_Font.c_str());
									pTextStyle->LinkEndChild(pFont);
									pFont->SetAttribute(
											Serializer_XML_Attribute_Name.c_str(),
											""); //TODO
								}
								//UsageType
								{
									XMLElement * pUsageType =
											pDoc.NewElement(
													Serializer_XML_Element_UsageType.c_str());
									pTextStyle->LinkEndChild(pUsageType);
									pUsageType->SetAttribute(
											Serializer_XML_Attribute_Name.c_str(),
											2); //TODO
								}
							}
							...
							//Location
							{
								XMLElement * pLocation =
										pDoc.NewElement(
												Serializer_XML_Element_Location.c_str());
								pText->LinkEndChild(pLocation);

								string temstr = M3DTOOLS::floatToString(
										textNote->GetNotePos().m_x) + " "
										+ M3DTOOLS::floatToString(
												textNote->GetNotePos().m_y)
										+ " "
										+ M3DTOOLS::floatToString(
												textNote->GetNotePos().m_z);
							...
							//Rotation
							{
								XMLElement * pRotation =
										pDoc.NewElement(
												Serializer_XML_Element_Rotation.c_str());
					...
					//OuterBox
					{
						XMLElement * pOuterBox = pDoc.NewElement(
								Serializer_XML_Element_OuterBox.c_str());
...
string NoteFactory::TextNoteToJSONValue(SceneManager* scene,
		TextNote* textNote) {
	string JSONValue = "";
...
Note* NoteFactory::CreateVoiceNoteFromXMLElement(SceneManager* scene,
		const string& xmlElement) {
	...
	XMLElement* pVoiceNote = doc.FirstChildElement(
			Serializer_XML_Element_VoiceNote.c_str());
	if (pVoiceNote)
		int id = pVoiceNote->IntAttribute(Serializer_XML_Attribute_ID.c_str());

	XMLElement * pLocation = pVoiceNote->FirstChildElement(
			Serializer_XML_Element_Location.c_str());
	if (pLocation) {
		...
		if (3 == floats.size()) {
			...
			Vector3 tempV(x, y, z);
			LOGE("POS %s", tempV.Tostring().c_str());
			Vector2 voiceNoteSize(1.0f, 1.0f);

			voiceNote = new VoiceNote(tempV,
					ShapeHelper::GetCommonSize(scene, voiceNoteSize), scene);

			voiceNote->SetPosition(tempV);
		}
	}
	XMLElement * pVoiceData = pVoiceNote->FirstChildElement(
			Serializer_XML_Element_VoiceData.c_str());
	if (pVoiceData && pVoiceData->FirstChild()) {
		string uri = pVoiceData->Attribute(
				Serializer_XML_Attribute_URI.c_str());
		voiceNote->SetUri(uri);
		LOGI("uri = %s", uri.c_str());

		string format = pVoiceData->Attribute(
				Serializer_XML_Attribute_Format.c_str());
	...
	//解析Attributes元素及子元素
	map<string, string> attributeList;
	XMLElement* xmlAttributes = pVoiceNote->FirstChildElement(
			Serializer_XML_Element_Attributes.c_str());
	if (xmlAttributes) {
		XMLElement* xmlAttribute = xmlAttributes->FirstChildElement(
				Serializer_XML_Element_Attribute.c_str());
...
string NoteFactory::VoiceNoteToXMLElement(SceneManager* scene,
		VoiceNote* voiceNote) {
	...
	XMLElement *pVoiceNote = pDoc.NewElement(
			Serializer_XML_Element_VoiceNote.c_str());
	pDoc.LinkEndChild(pVoiceNote);
	pVoiceNote->SetAttribute(Serializer_XML_Attribute_ID.c_str(),
			voiceNote->GetID());

	{
		...
		string locationText = M3DTOOLS::floatToString(X) + " "
				+ M3DTOOLS::floatToString(Y) + " " + M3DTOOLS::floatToString(Z);

		LOGE("locationText %s", locationText.c_str());
		XMLText * pLocationText = pDoc.NewText(locationText.c_str());
		pLocation->LinkEndChild(pLocationText);
	}
	{
		XMLElement * pVoiceData = pDoc.NewElement(
				Serializer_XML_Element_VoiceData.c_str());
		pVoiceNote->LinkEndChild(pVoiceData);

		pVoiceData->SetAttribute(Serializer_XML_Attribute_URI.c_str(),
				voiceNote->GetUri().c_str()); //TODO
		pVoiceData->SetAttribute(Serializer_XML_Attribute_Format.c_str(),
				voiceNote->GetFormat().c_str()); //TODO

		XMLText * pVoiceDataText = pDoc.NewText(
				voiceNote->GetVoiceData().c_str());
...

	{
		...
		for (map<string, string>::iterator it =
				voiceNote->GetPropertyMap().begin();
...
Note* NoteFactory::CreatePntTextNote(int firtShapId, Vector2& screenPnt,
		const string& text, SceneManager* scene, Properties* properties) {
	TextNote* note = NULL;
//
	Shape* firstShape = scene->GetShape(firtShapId);
	if (!firstShape) {
	...
	if (firstShape && firstShape->GetType() == SHAPE_POINT_HANDLE) {
		...
		if (scene->GetPickPoint(screenPnt, anchorCoordiante, false)) {
			...
			ImageBoard * imageBroad =
					MeasureDisplayHelper::createNoteTextsImageN(scene, temptext,
							pntInPlane);
...
Note*
NoteFactory::CreatePntSequenceNumberNote(int notePointShapId,
		Vector2& screenPnt, const string& text, SceneManager* scene,
		Properties* properties) {
	SequenceNumberNote* note = NULL;
//
	Shape* notePointShape = scene->GetShape(notePointShapId);
	if (!notePointShape) {
	...
	if (notePointShape && notePointShape->GetType() == SHAPE_POINT_HANDLE) {
		...
		if (scene->GetPickPoint(screenPnt, anchorCoordiante, false)) {
			...
			ImageBoard * imageBroad =
					MeasureDisplayHelper::CreateSequenceNumberImage(scene,
							temptext, pntInPlane);
...
Note* NoteFactory::CreatePntVoiceNote(Vector2& screenPnt, const string& text,
		SceneManager* scene) {
	Note* note = NULL;
	Vector3 anchorCoordiante;
	LOGE("");
	if (scene->GetPickPoint(screenPnt, anchorCoordiante, false)) {
		Vector2 voiceNoteSize(1.0f, 1.0f);
		note = new VoiceNote(anchorCoordiante,
				ShapeHelper::GetCommonSize(scene, voiceNoteSize), scene);
	}
...
Note* NoteFactory::CreateSequenceNoteFromXMLElement(SceneManager* scene,
		const string& xmlElement) {
	...
	XMLElement* xmlNote = doc.FirstChildElement(
			Serializer_XML_Element_SequenceNote.c_str());
	...
	ImageBoard * imageBord = MeasureDisplayHelper::CreateSequenceNumberImage(
			scene, contents, endPnt);
	LOGE("ImageBoard pointer %p", imageBord);
...
string NoteFactory::SequenceNoteToXMLElement(SceneManager* scene,
		SequenceNumberNote* sequenceNote) {
	LOGI("begin serialize");
	//序列化
	string xmlElement = "";
	...
	XMLDocument pDoc; //创建文档
	//创建根节点
	XMLElement *pSequenceNote = pDoc.NewElement(
			Serializer_XML_Element_SequenceNote.c_str());
	pDoc.LinkEndChild(pSequenceNote);
	pSequenceNote->SetAttribute(Serializer_XML_Attribute_ID.c_str(),
			sequenceNote->GetID());
	{
		//Leaders节点
		XMLElement * pLeaders = pDoc.NewElement(
...

			{
						...
						0); //TODO:
				{
					...
					//创建Point
					{
						...
						pPolyline->LinkEndChild(pPoint);
						pPoint->SetAttribute(Serializer_XML_Attribute_X.c_str(),
								sequenceNote->GetNotePos().m_x); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
								sequenceNote->GetNotePos().m_y); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
								sequenceNote->GetNotePos().m_z); //TODO
					}
					...
					//创建Point
					{
						...
						pPolyline->LinkEndChild(pPoint);
						pPoint->SetAttribute(Serializer_XML_Attribute_X.c_str(),
								sequenceNote->GetTextsPos().m_x); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Y.c_str(),
								sequenceNote->GetTextsPos().m_y); //TODO
						pPoint->SetAttribute(Serializer_XML_Attribute_Z.c_str(),
								sequenceNote->GetTextsPos().m_z); //TODO
					}
...
//			}
					...
					pCompositeText->LinkEndChild(pTexts);
					{
								...
								sequenceNote->m_ComTexts.at(0)->GetCText(0)->GetText().c_str());
						{
							//Location
							{
								...
								string temstr = M3DTOOLS::floatToString(
										sequenceNote->GetNotePos().m_x) + " "
										+ M3DTOOLS::floatToString(
												sequenceNote->GetNotePos().m_y)
										+ " "
										+ M3DTOOLS::floatToString(
												sequenceNote->GetNotePos().m_z);
								tinyxml2::XMLText * pContent = pDoc.NewText(
										temstr.c_str());
								pLocation->LinkEndChild(pContent);
...

		{
			...
			for (map<string, string>::iterator it =
					sequenceNote->GetPropertyMap().begin();
