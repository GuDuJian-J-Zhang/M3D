...
SceneNode::SceneNode(void):Object()
{
	m_pParent = NULL;
...
void SceneNode::SetVisible(bool isHide)
{
	this->m_bVisible = isHide;
}
...
void SceneNode::SetTransform(const Vector3& position, const Quaternion& rotation)
{
    m_position = position;
    m_rotation = rotation;
    MarkDirty();
...
void SceneNode::SetTransform(const Vector3& position, const Quaternion& rotation, float scale)
{
    SetTransform(position, rotation, Vector3(scale, scale, scale));
}

void SceneNode::SetTransform(const Vector3& position, const Quaternion& rotation, const Vector3& scale)
{
    m_position = position;
    m_rotation = rotation;
    m_scale = scale;
...
{
    SetPosition((m_pParent == NULL) ? position : m_pParent->GetWorldTransform().Inverse() * position);
}

void SceneNode::SetWorldRotation(const Quaternion& rotation)
{
    SetRotation((m_pParent == NULL) ? rotation : m_pParent->GetWorldRotation().Inverse() * rotation);
}

void SceneNode::SetWorldDirection(const Vector3& direction)
{
    Vector3 localDirection = (m_pParent == NULL ) ? direction : m_pParent->GetWorldRotation().Inverse() * direction;
...
void SceneNode::SetWorldTransform(const Vector3& position, const Quaternion& rotation)
{
    SetWorldPosition(position);
    SetWorldRotation(rotation);
}

void SceneNode::SetWorldTransform(const Vector3& position, const Quaternion& rotation, float scale)
{
    SetWorldPosition(position);
    SetWorldRotation(rotation);
    SetWorldScale(scale);
...
void SceneNode::SetWorldTransform(const Vector3& position, const Quaternion& rotation, const Vector3& scale)
{
    SetWorldPosition(position);
    SetWorldRotation(rotation);
    SetWorldScale(scale);
...
void SceneNode::Translate(const Vector3& delta, TransformSpace space)
{
    ...
    case TS_WORLD:
        m_position += (m_pParent == NULL ) ? delta : m_pParent->GetWorldTransform().Inverse() * Vector4(delta, 0);
        break;
    }
    MarkDirty();
}
...
void SceneNode::RotateAround(const Vector3& point, const Quaternion& delta, TransformSpace space)
{
...
bool SceneNode::LookAt(const Vector3& target, const Vector3& up, TransformSpace space)
{
    ...
    case TS_PARENT:
        worldSpaceTarget = (m_pParent == NULL) ? target : m_pParent->GetWorldTransform() * target;
        break;
...
void SceneNode::Scale(const Vector3&  delta)
{
	m_scale *= delta;
    MarkDirty();
}
