...
namespace M3D
{
  ...
  ModelView::ModelView(ModelView & modelView):Object()
  {
	  LOGI("cope  ModelView BEgin");
	    ...
	    LOGI(" this-> m_UpDataModel=modelView.m_UpDataModel;");
...
//	}
  ...
  void
  ModelView::AddSectionPlaneId(int id)
  {
    bool exist = false;
  ...
  vector<int>
  ModelView::GetNoteList()
  {
    return m_NoteList;
  }

  bool
  ModelView::GetUpDataModelState()
  {
    return m_UpDataModel;
  }

  void
  ModelView::SetUpDataModel(bool state)
  {
    m_UpDataModel = state;
  }

  bool
  ModelView::GetUpDataCameraState()
  {
    return m_UpDataCamera;
  }

  void
  ModelView::SetUpDataCamera(bool state)
  {
    m_UpDataCamera = state;
  }

  vector<int>
  ModelView::GetPMIList()
  {
    return m_PMIList;
  }

  vector<int>
  ModelView::GetSectionPlaneIDList()
  {
     ...
     * @param flag
     */
    void ModelView::SetShowCutSectionPlane(bool flag){
        m_ShowCutPlane = flag;
    }
    /**
     * 获取剖切面显示标示
     *
     */
    bool ModelView::GetShowCutSectionPlane(){
        return m_ShowCutPlane;
    }
    /**
     * 获取剖切面的辅助面显示标示
     *
  ...
  void
  ModelView::AddNoteId(int id)
  {
    m_NoteList.push_back(id);
  }

  void
  ModelView::SetNoteIds(vector<int> ids)
  {
    m_NoteList.assign(ids.begin(), ids.end());
  }

  void
  ModelView::AddPMIId(int id)
  {
    m_PMIList.push_back(id);
  }

  void
  ModelView::SetPMIIds(vector<int> ids)
  {
    m_PMIList.assign(ids.begin(), ids.end());
  }

  void
  ModelView::SetCamera(CameraNode &camera)
  {
    m_Camera = camera;
  }

  void
  ModelView::SetName(const char* name)
  {
    m_Name = name;
  }

  string
  ModelView::GetProperty(string key)
  {
  	string ret = "";
  ...
  void
  ModelView::SetProperty(string key ,string value)
  {
  	m_appendInfo.insert(make_pair(key,value));
  }
  void
  ModelView::ClearProperties()
  {
  	m_appendInfo.erase(m_appendInfo.begin(),m_appendInfo.end());
  }

  bool
  ModelView::DeleteProperty(string key)
  {
  	bool ret;
  	map<string, string>::iterator it;
  	it = m_appendInfo.find(key);
...
void
ModelView::SetNoteDataList(SHAPETYPE key, vector<string>& noteDataList)
{
