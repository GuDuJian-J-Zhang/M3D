...
VHASH_API void VHASH_CDECL vhash_flush(vhash_t * v)
{ 
	vhash_clear(v);
    memset(v->table, 0, sizeof(vhash_node_t)*v->table_size);
    v->unique_count = 0;
...
VHASH_API void VHASH_CDECL delete_vhash(vhash_t * v)
{
	vhash_clear(v);
...
#ifdef SVIEW_DEFINED
VHASH_API Memory_Pool * VHASH_CDECL vhash_memory_pool(vhash_t * v)
{
	return v->memory_pool;
}
#else
VHASH_API vmalloc_t VHASH_CDECL vhash_malloc(vhash_t * v)
{
	return v->vmalloc;
}

VHASH_API vfree_t VHASH_CDECL vhash_free(vhash_t * v)
{
	return v->vfree;
}
#endif
...
VHASH_API void VHASH_CDECL vhash_insert(
	vhash_t *		v,
	void *			in_key, 
	void *			item) 
{
...
VHASH_API void * VHASH_CDECL vhash_remove(
	vhash_t *			v,
	void *				in_key) 
{ 
    void * item = 0;
...
VHASH_API void * VHASH_CDECL vhash_lookup(
	vhash_t *			v,
	void *				in_key) 
{ 
    void * item = 0;
...
VHASH_API void * VHASH_CDECL vhash_lookup_string_key(
	vhash_t *			v,
	const char *		string_key)
{
    void * item = 0;
...
VHASH_API void VHASH_CDECL vhash_insert_string_key(
	vhash_t *			v,
	const char *		string, 
	void *				item) 
{ 
...
VHASH_API void * VHASH_CDECL vhash_remove_string_key(
    vhash_t *			v,
    const char *		string)
{
    void * item = 0;
...
VHASH_API void VHASH_CDECL vhash_map_function_with_return(
    ...
    void *					user_data)
{
	vhash_map_return_t		ret = 0;
	unsigned long			i;
    
    for (i=0; i<v->table_size; i++) {
	    if (NODE_STATE_IN_USE(v->table[i])) {
			if (v->table[i].count == 1) {
				ret = (*function)(v->table[i].item, (void *) v->table[i].key, user_data);
				...
				void **		items = (void **)v->table[i].item;
				int			j, k;
				bool		*pending_removals = 0;
				bool		nominal_pending_removal_buffer[PRB_NOMINAL_SIZE];
				int			pending_removal_count = 0;

				for (j=0; j<v->table[i].count; j++) {
					ret = (*function)(items[j], (void *) v->table[i].key, user_data);
					if (ret & VHASH_MAP_RETURN_DELETE) {
						if (!pending_removals) {
							if (v->table[i].count > PRB_NOMINAL_SIZE)
								VMALLOC_ARRAY(v, pending_removals, v->table[i].count, bool);
							else
								pending_removals = nominal_pending_removal_buffer;
							memset (pending_removals, 0, sizeof(bool)*v->table[i].count);
						}
						pending_removals[j] = true;
				...
				if (pending_removals) {
					void **victim = (void **)v->table[i].item;
					int count = v->table[i].count;

					if (count - pending_removal_count > 1) {
						VMALLOC_ARRAY (v, items, count - pending_removal_count, void *);
						v->table[i].item = (void *)items;
						k = 0;
						for (j = 0 ; j < count ; j++) {
							if (!pending_removals[j]) {
								items[k] = victim[j];
								k++;
							}
						}
						ASSERT (k == count - pending_removal_count);
					}
					else if (count - pending_removal_count == 1) {
						for (j = 0 ; j < count ; j++) {
							if (!pending_removals[j]) {
								v->table[i].item = victim[j];
								break;
							}
						}
					}

					VFREE_ARRAY(v, victim, count, void *);
					if (pending_removals != nominal_pending_removal_buffer)
						VFREE_ARRAY(v, pending_removals, count, bool);
					v->key_count -= pending_removal_count;
					v->table[i].count -= pending_removal_count;
					if (v->table[i].count == 0) {
						MARK_OLD_NODE(v->table[i]);
						v->unique_count--;
					}
				}
