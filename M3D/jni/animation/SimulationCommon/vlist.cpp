...
VLIST_API bool VLIST_CDECL vlist_move_item(
	...
	const int newIdx)
{
	vlist_node_t* node = NULL;
	vlist_node_t* new_pre_node = NULL;
	vlist_node_t* old_pre_node = NULL;
	if(oldIdx < 0 || oldIdx >= vlist_count(vlist) ||
		newIdx < 0 || newIdx >= vlist_count(vlist))
	{
		return false;
	}
	vlist_nth_item(vlist,oldIdx);
	node = vlist->cursor;

	if(newIdx >=0)
	{
		vlist_nth_item(vlist,newIdx);
		new_pre_node = vlist->cursor;
	}
	if(oldIdx >0)
	{
		vlist_nth_item(vlist,oldIdx-1);
		old_pre_node = vlist->cursor;
	}

	if(old_pre_node && node )
	{
		old_pre_node->next = node->next;
	}
	else
	{
		vlist->head = node->next;
	...
	if(new_pre_node && node )
	{
		node->next = new_pre_node->next;
		new_pre_node->next = node;
	}
	else
	{
		node->next = vlist->head;
	...
	VLIST_RESET_CURSOR(vlist)
	if(oldIdx >= (vlist_count(vlist) - 1) ||
		newIdx >= (vlist_count(vlist) - 1))
	{
		vlist_nth_item(vlist,vlist_count(vlist) - 1);
		node = vlist->cursor;
		vlist->tail=node;
	}
