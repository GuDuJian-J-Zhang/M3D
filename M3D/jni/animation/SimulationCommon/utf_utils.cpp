...
#define UTF_DEFAULT_LOCALE(x) do{char const * const oldloc = setlocale(LC_CTYPE, ""); (x); setlocale(LC_CTYPE, oldloc);}while(0)
...
SA_UTF32::SA_UTF32(SA_UTF32 const & rhs) : m_utf32(0), m_size(0) {
  if(rhs.encodedText()){
...
SA_UTF32 const & SA_UTF32::operator = (SA_UTF32 const & rhs) {
    delete [] m_utf32;
    m_utf32 = 0;
    m_size = 0;
    if(rhs.encodedText()){
...
SA_UTF32::SA_UTF32() : m_utf32(0), m_size(0) {}

SA_UTF32::SA_UTF32(wchar_t const * text) : m_utf32(0), m_size(0) {
    if(text){
...
#ifdef _MSC_VER
SA_UTF32::SA_UTF32(unsigned short const * text) : m_utf32(0), m_size(0) {
    if(text){
		if(sizeof(wchar_t) == sizeof(utf32_char)){
...
SA_UTF32::SA_UTF32(char const * text) : m_utf32(0), m_size(0) {
    if(text){
        UTF_DEFAULT_LOCALE(m_size = mbstowcs(0, text, 0) +1);
        wchar_t * const tmp = new wchar_t[m_size];
        UTF_DEFAULT_LOCALE(mbstowcs(tmp, text, m_size));
        *this = SA_UTF32(tmp);
        delete [] tmp;
    }
}
...
SA_UTF32::SA_UTF32(SA_UTF16 const & unicode_text) : m_utf32(0), m_size(0) {
...
SA_UTF32::SA_UTF32(SA_UTF8 const & unicode_text) : m_utf32(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf32_char_count(unicode_text.begin());
        m_utf32 = new utf32_char[m_size];
        unicode_to_utf32(m_utf32, unicode_text.begin());
...
SA_UTF32::SA_UTF32(SA_URI const & unicode_text) : m_utf32(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf32_char_count(unicode_text.begin());
        m_utf32 = new utf32_char[m_size];
        unicode_to_utf32(m_utf32, unicode_text.begin());
...
SA_UTF32::~SA_UTF32() {delete [] m_utf32;}

size_t SA_UTF32::length() const {
	if(m_utf32)
	  return (size_t)(utf32_char_count(begin()) -1);
	return 0;
}

SA_UTF32 & SA_UTF32::format(wchar_t const * fmt, ...){
  va_list va[2];
...
#ifdef _MSC_VER
SA_UTF32 & SA_UTF32::format(unsigned short const * fmt, ...){
  va_list va[2];
...
SA_UTF32 & SA_UTF32::format(char const * fmt, ...){
  va_list va[2];
...
SA_UTF32 & SA_UTF32::append(wchar_t const * txt){
  SA_WCS orig_utf(*this);
...
#ifdef _MSC_VER
SA_UTF32 & SA_UTF32::append(unsigned short const * txt){
	return append((wchar_t const*) txt);
}
#endif

SA_UTF32 & SA_UTF32::append(char const * txt){
  SA_WCS wcs(txt);
  append(wcs.encodedText());
  return *this;
}

int SA_UTF32::iterator::get_and_advance(utf32_char const * & text) const {
    int code_point = *text++;
    return code_point;
};

SA_UTF32::iterator::iterator(utf32_char const * text) : m_text(text) {
}

SA_UTF32::iterator SA_UTF32::iterator::operator ++ () const {
    return iterator(++m_text);
}

SA_UTF32::iterator SA_UTF32::iterator::operator ++ (int) const {
    return iterator(m_text++);
}
int SA_UTF32::iterator::operator * () const {
    utf32_char const * tmp = m_text;
    int code_point = *tmp;
    return code_point;
}
SA_UTF32::iterator::operator bool () const {
    return false;
}

SA_UTF32::iterator SA_UTF32::begin() const {return iterator(m_utf32);}
SA_UTF32::iterator SA_UTF32::end() const {return iterator(m_utf32 + m_size);}

bool SA_UTF32::operator == (SA_UTF32 const & rhs) const
{
...
SA_UTF16::SA_UTF16(SA_UTF16 const & rhs) : m_utf16(0), m_size(0) {
    if(rhs.encodedText()){
...
SA_UTF16 const & SA_UTF16::operator = (SA_UTF16 const & rhs) {
    delete [] m_utf16;
    m_utf16 = 0;
    m_size = 0;
    if(rhs.encodedText()){
...
SA_UTF16::SA_UTF16() : m_utf16(0), m_size(0) {}

SA_UTF16::SA_UTF16(wchar_t const * text) : m_utf16(0), m_size(0) {
    if(text){
...
#ifdef _MSC_VER
SA_UTF16::SA_UTF16(unsigned short const * text) : m_utf16(0), m_size(0) {
    if(text){
        if(sizeof(wchar_t) == sizeof(utf16_char)){
...
SA_UTF16::SA_UTF16(char const * text) : m_utf16(0), m_size(0) {
    if(text){
        UTF_DEFAULT_LOCALE(m_size = mbstowcs(0, text, 0) +1);
        wchar_t * const tmp = new wchar_t[m_size];
        UTF_DEFAULT_LOCALE(mbstowcs(tmp, text, m_size));
...
SA_UTF16::SA_UTF16(SA_UTF32 const & unicode_text) : m_utf16(0), m_size(0) {
    if(unicode_text.encodedText()){
...
SA_UTF16::SA_UTF16(SA_UTF8 const & unicode_text) : m_utf16(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf16_char_count(unicode_text.begin());
        m_utf16 = new utf16_char[m_size];
        unicode_to_utf16(m_utf16, unicode_text.begin());
...
SA_UTF16::SA_UTF16(SA_URI const & unicode_text) : m_utf16(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf16_char_count(unicode_text.begin());
        m_utf16 = new utf16_char[m_size];
        unicode_to_utf16(m_utf16, unicode_text.begin());
...
SA_UTF16::~SA_UTF16() {delete [] m_utf16;}

size_t SA_UTF16::length() const {
	if(m_utf16)
	  return (size_t)(utf16_char_count(begin()) -1);
	return 0;
}

SA_UTF16 & SA_UTF16::format(wchar_t const * fmt, ...){
  va_list va[2];
...
#ifdef _MSC_VER
SA_UTF16 & SA_UTF16::format(unsigned short const * fmt, ...){
  va_list va[2];
...
SA_UTF16 & SA_UTF16::format(char const * fmt, ...){
  va_list va[2];
...
SA_UTF16 & SA_UTF16::append(wchar_t const * txt){
  SA_WCS orig_utf(*this);
  wchar_t const * orig_wcs = orig_utf.encodedText();
  if(!orig_wcs)
	  orig_wcs = L"";
...
#ifdef _MSC_VER
SA_UTF16 & SA_UTF16::append(unsigned short const * txt){
	return append((wchar_t const*) txt);
}
#endif

SA_UTF16 & SA_UTF16::append(char const * txt){
  SA_WCS wcs(txt);
  append(wcs.encodedText());
  return *this;
}

int SA_UTF16::iterator::get_and_advance(utf16_char const * & text) const {
    int w1 = *text++;
    if(w1 >= 0 && w1 <= 0xffff){
        return w1;
    }
...
SA_UTF16::iterator::iterator(utf16_char const * text) : m_text(text) {
}

SA_UTF16::iterator SA_UTF16::iterator::operator ++ () const {
    get_and_advance(m_text);
    return iterator(m_text);
}

SA_UTF16::iterator SA_UTF16::iterator::operator ++ (int) const {
    iterator retval(m_text);
    get_and_advance(m_text);
    return retval;
}

int SA_UTF16::iterator::operator * () const {
    utf16_char const * tmp = m_text;
    return get_and_advance(tmp);
}

SA_UTF16::iterator::operator bool () const {
    return false;
}

SA_UTF16::iterator SA_UTF16::begin() const {return iterator(m_utf16);}
SA_UTF16::iterator SA_UTF16::end() const {return iterator(m_utf16 + m_size);}

bool SA_UTF16::operator == (SA_UTF16 const & rhs) const
{
  if(length() != rhs.length())
    return false;
...
SA_UTF8::SA_UTF8(SA_UTF8 const & rhs) : m_utf8(0), m_size(0) {
    if(rhs.encodedText()){
...
SA_UTF8 const & SA_UTF8::operator = (SA_UTF8 const & rhs) {
    delete [] m_utf8;
    m_utf8 = 0;
    m_size = 0;
    if(rhs.encodedText()){
...
SA_UTF8::SA_UTF8() : m_utf8(0), m_size(0) {}

SA_UTF8::SA_UTF8(wchar_t const * text) : m_utf8(0), m_size(0) {
    if(text){
        SA_WCS u;
		u.encodedText(text);
...
#ifdef _MSC_VER
SA_UTF8::SA_UTF8(unsigned short const * text) : m_utf8(0), m_size(0) {
    if(text){
        SA_WCS u;
...
SA_UTF8::SA_UTF8(char const * text) : m_utf8(0), m_size(0) {
    if(text){
        UTF_DEFAULT_LOCALE(m_size = mbstowcs(0, text, 0) +1);
        wchar_t * const tmp = new wchar_t[m_size];
        UTF_DEFAULT_LOCALE(mbstowcs(tmp, text, m_size));
...
SA_UTF8::SA_UTF8(SA_UTF32 const & unicode_text) : m_utf8(0), m_size(0) {
    if(unicode_text.encodedText()){
...
SA_UTF8::SA_UTF8(SA_UTF16 const & unicode_text) : m_utf8(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf8_char_count(unicode_text.begin());
        m_utf8 = new utf8_char[m_size];
        unicode_to_utf8(m_utf8, unicode_text.begin());
...
SA_UTF8::SA_UTF8(SA_URI const & unicode_text) : m_utf8(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = utf8_char_count(unicode_text.begin());
        m_utf8 = new utf8_char[m_size];
        unicode_to_utf8(m_utf8, unicode_text.begin());
...
SA_UTF8::~SA_UTF8() {delete [] m_utf8;}

size_t SA_UTF8::length() const {
	if(m_utf8)
	  return (size_t)(utf8_char_count(begin()) -1);
	return 0;
}

SA_UTF8 & SA_UTF8::format(wchar_t const * fmt, ...){
  va_list va[2];
...
#ifdef _MSC_VER
SA_UTF8 & SA_UTF8::format(unsigned short const * fmt, ...){
  va_list va[2];
...
SA_UTF8 & SA_UTF8::format(char const * fmt, ...){
  va_list va[2];
...
SA_UTF8 & SA_UTF8::append(wchar_t const * txt){
  SA_WCS orig_utf(*this);
  wchar_t const * orig_wcs = orig_utf.encodedText();
  if(!orig_wcs)
	  orig_wcs = L"";
...
#ifdef _MSC_VER
SA_UTF8 & SA_UTF8::append(unsigned short const * txt){
  return append((wchar_t const*) txt);
}
#endif

SA_UTF8 & SA_UTF8::append(char const * txt){
  SA_WCS wcs(txt);
  append(wcs.encodedText());
  return *this;
}

int SA_UTF8::iterator::get_and_advance(utf8_char const * & text) const {
    int w1 = *text++;
    if(w1 < 0x7f){
        return w1;
    }
...
SA_UTF8::iterator::iterator(utf8_char const * text) : m_text(text) {
}

SA_UTF8::iterator SA_UTF8::iterator::operator ++ () const {
    get_and_advance(m_text);
    return iterator(m_text);
}

SA_UTF8::iterator SA_UTF8::iterator::operator ++ (int) const {
    iterator retval(m_text);
    get_and_advance(m_text);
    return retval;
}

int SA_UTF8::iterator::operator * () const {
    utf8_char const * tmp = m_text;
    return get_and_advance(tmp);
}

SA_UTF8::iterator::operator bool () const {
    return false;
}

SA_UTF8::iterator SA_UTF8::begin() const {return iterator(m_utf8);}
SA_UTF8::iterator SA_UTF8::end() const {return iterator(m_utf8 + m_size);}

bool SA_UTF8::operator == (SA_UTF8 const & rhs) const
{
  if(length() != rhs.length())
    return false;
...
SA_URI::SA_URI(SA_URI const & rhs) : m_uri(0), m_size(0) {
    if(rhs.encodedText()){
...
SA_URI const & SA_URI::operator = (SA_URI const & rhs) {
    delete [] m_uri;
    m_uri = 0;
    m_size = 0;
    if(rhs.encodedText()){
...
SA_URI::SA_URI() : m_uri(0), m_size(0) {}

SA_URI::SA_URI(wchar_t const * text) : m_uri(0), m_size(0) {
    if(text){
        SA_WCS u;
	u.encodedText(text);
...
#ifdef _MSC_VER
SA_URI::SA_URI(unsigned short const * text) : m_uri(0), m_size(0) {
    if(text){
        SA_WCS u;
	u.encodedText((wchar_t const*) text);
        m_size = uri_char_count(u.begin());
...
/* This is locale specific multibyte string text. */
SA_URI::SA_URI(char const * text) : m_uri(0), m_size(0) {
    if(text){
        UTF_DEFAULT_LOCALE(m_size = mbstowcs(0, text, 0) +1);
        wchar_t * const tmp = new wchar_t[m_size];
        UTF_DEFAULT_LOCALE(mbstowcs(tmp, text, m_size));
...
SA_URI::SA_URI(SA_UTF32 const & unicode_text) : m_uri(0), m_size(0) {
    if(unicode_text.encodedText()){
...
SA_URI::SA_URI(SA_UTF16 const & unicode_text) : m_uri(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = uri_char_count(unicode_text.begin());
        m_uri = new utf8_char[m_size];
        unicode_to_uri(m_uri, unicode_text.begin());
...
SA_URI::SA_URI(SA_UTF8 const & unicode_text) : m_uri(0), m_size(0) {
    if(unicode_text.encodedText()){
        m_size = uri_char_count(unicode_text.begin());
        m_uri = new utf8_char[m_size];
        unicode_to_uri(m_uri, unicode_text.begin());
...
SA_URI::~SA_URI() {delete [] m_uri;}

size_t SA_URI::length() const {
	if(m_uri)
	  return (size_t)(uri_char_count(begin()) -1);
	return 0;
}

SA_URI & SA_URI::format(wchar_t const * fmt, ...){
  va_list va[2];
...
#ifdef _MSC_VER
SA_URI & SA_URI::format(unsigned short const * fmt, ...){
  va_list va[2];
...
SA_URI & SA_URI::format(char const * fmt, ...){
  va_list va[2];
...
SA_URI & SA_URI::append(wchar_t const * txt){
  SA_WCS orig_utf(*this);
  wchar_t const * orig_wcs = orig_utf.encodedText();
  if(!orig_wcs)
	  orig_wcs = L"";
...
#ifdef _MSC_VER
SA_URI & SA_URI::append(unsigned short const * txt){
  return append((wchar_t const*) txt);
}
#endif

SA_URI & SA_URI::append(char const * txt){
  SA_WCS wcs(txt);
  append(wcs.encodedText());
  return *this;
}

int SA_URI::iterator::get_and_advance(utf8_char const * & text) const {
    int w1 = *text++;
    if(w1 != '%'){
        return w1;
    }
...
SA_URI::iterator::iterator(utf8_char const * text) : m_text(text) {
}

SA_URI::iterator SA_URI::iterator::operator ++ () const {
    get_and_advance(m_text);
    return iterator(m_text);
}

SA_URI::iterator SA_URI::iterator::operator ++ (int) const {
    iterator retval(m_text);
    get_and_advance(m_text);
    return retval;
}

int SA_URI::iterator::operator * () const {
    utf8_char const * tmp = m_text;
    return get_and_advance(tmp);
}

SA_URI::iterator::operator bool () const {
    return false;
}

SA_URI::iterator SA_URI::begin() const {return iterator(m_uri);}
SA_URI::iterator SA_URI::end() const {return iterator(m_uri + m_size);}

bool SA_URI::operator == (SA_URI const & rhs) const
{
  if(length() != rhs.length())
    return false;
...
char * _va_format_mbs(char const * const format, va_list va[])
{
    ...
    UTF_DEFAULT_LOCALE(format_count = mbstowcs(0, format, 0));
    wchar_t * const wcs_format = new wchar_t[format_count +1];
    UTF_DEFAULT_LOCALE(mbstowcs(wcs_format, format, format_count +1));
    ...
    if(wcs_count >= 0){
        int mbs_count = 0;
		UTF_DEFAULT_LOCALE(mbs_count = (int)wcstombs(0, wcs, 0));
        if(mbs_count >= 0){
			retval = new char[mbs_count +1];
			UTF_DEFAULT_LOCALE(wcstombs(retval, wcs, mbs_count +1));
		}
		else{
...
#ifdef _MSC_VER
SAUtilityUnicodeStr::SAUtilityUnicodeStr(const unsigned short *p16) : m_wcs((wchar_t const*) p16) {}
#endif

SAUtilityUnicodeStr::SAUtilityUnicodeStr(const wchar_t *p16) : m_wcs(p16) {}

SAUtilityUnicodeStr::operator const wchar_t * (void) const
{
	return m_wcs.encodedText();
}

#ifdef _MSC_VER
SAUtilityUnicodeStr::operator const unsigned short * (void) const
{
	return (unsigned short const*)m_wcs.encodedText();
}
#endif

SAUtilityUnicodeStr::SAUtilityUnicodeStr(const char *p8) : m_wcs(p8) {}

SAUtilityUnicodeStr::~SAUtilityUnicodeStr() {}

SAUtilityAsciiStr::SAUtilityAsciiStr(const char *p8) : m_sz(0)
{
	if(p8){
...
SAUtilityAsciiStr::operator const char * (void) const 
{
	return m_sz;
}

#ifdef _MSC_VER
SAUtilityAsciiStr::SAUtilityAsciiStr(const unsigned short *p16) : m_sz(0)
{
	if(p16)
	{
...
SAUtilityAsciiStr::SAUtilityAsciiStr(const wchar_t *p16) : m_sz(0)
{
	if(p16)
	{
		// calculate string length
...
SAUtilityAsciiStr::~SAUtilityAsciiStr(void) {
	delete [] m_sz;
}

int const SA_FORMAT_TEXT::sa_vsnprintf(char * const buf, size_t const n, char const * const format, va_list valist)
{
#ifdef _MSC_VER
...
SA_FORMAT_TEXT::SA_FORMAT_TEXT(char const * const format, ...) : m_str(0) {
	int result = 0;
	...
	do{
		...
		UTF_DEFAULT_LOCALE( (result = SA_FORMAT_TEXT::sa_vsnprintf(m_str, count, format, va1)) );
...
SA_FORMAT_TEXT & SA_FORMAT_TEXT::Append(char const * format, ...) {
	int count = (int)strlen(format) + 256;
	int result = 0;

	SA_FORMAT_TEXT mod_format("%s%s", m_str, format);
	...
	do{
		...
		UTF_DEFAULT_LOCALE( (result = SA_FORMAT_TEXT::sa_vsnprintf(m_str, count, mod_format, va1)) );
...
SA_FORMAT_TEXT::~SA_FORMAT_TEXT() {
	free(m_str);
}

SA_FORMAT_TEXT::operator char * () const {
	return m_str;
}

SA_FORMAT_TEXT const & SA_FORMAT_TEXT::operator=(SA_FORMAT_TEXT const & rhs) {
	free(m_str);
	m_str = strdup(rhs.m_str);
	return *this;
}

SA_FORMAT_TEXT::SA_FORMAT_TEXT(SA_FORMAT_TEXT const & rhs) : m_str(strdup(rhs.m_str)){}

SA_FORMAT_TEXT::SA_FORMAT_TEXT() : m_str(strdup("")) {}
...
FILE * wfopen(wchar_t const * wcfilename, wchar_t const * wcmode) {
    ...
    if(wcfilename && wcmode){
        size_t size = 0;
	UTF_DEFAULT_LOCALE(size = wcstombs(0, wcfilename, 0));
        if(size > 0){
            ++size; /* for the null */
            char * const mbfilename = (char * const)malloc( size * sizeof(char) );
            if(mbfilename){
	      int result = 0;
	      UTF_DEFAULT_LOCALE(result = wcstombs(mbfilename, wcfilename, size));
                if(result != -1){
                    char mbmode[10] = {""};
                    UTF_DEFAULT_LOCALE(wcstombs(mbmode, wcmode, 10));
