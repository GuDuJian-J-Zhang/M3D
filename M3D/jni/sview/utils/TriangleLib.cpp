...
/*   single or float precision internally.  On 80x86 processors, this may   */
...
struct memorypool
{
	VOID **firstblock, **nowblock;VOID *nextitem;VOID *deaditemstack;VOID **pathblock;VOID *pathitem;
	int alignbytes;
	int itembytes;
	int itemsperblock;
	int itemsfirstblock;
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, float area)
...
//float area;                                      /* The area of the triangle. */
//#endif /* not ANSI_DECLARATORS */

{
	float dxoa, dxda, dxod;
	float dyoa, dyda, dyod;
	float oalen, dalen, odlen;
	float maxlen;
...
void info()
{
...
#else /* not SINGLE */
	printf("This executable is compiled for float precision arithmetic.\n\n\n");
#endif /* not SINGLE */
	printf(
	...
	printf(
			"  underflow and overflow.  If Triangle is compiled for float precision\n");
	printf(
	...
	printf(
			"    arithmetic, you might do better by recompiling it for float-precision.\n"
	);
	printf(
	...
	printf(
			"    above.)  Again, I recommend compiling Triangle for float (rather\n");
		...
		for (j = STARTINDEX; argv[i][j] != '\0'; j++)
		{
			...
			if (argv[i][j] == 'q')
			{
				...
				if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) || (argv[i][j + 1] == '.'))
				{
					...
					workstring[k] = '\0';
					b->minangle = (float) strtod(workstring, (char **) NULL);
				}
				else
				{
					b->minangle = 20.0;
			...
			if (argv[i][j] == 'a')
			{
				...
				if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) || (argv[i][j + 1] == '.'))
				{
					...
					workstring[k] = '\0';
					b->maxarea = (float) strtod(workstring, (char **) NULL);
					if (b->maxarea <= 0.0)
					{
...

{
	...
	if (printvertex == (vertex) NULL)
		...
		printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n", t->orient + 3, (unsigned long) printvertex, printvertex[0],
				printvertex[1]);
...

{
	...
	if (printvertex == (vertex) NULL)
		...
		printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n", 2 + s->ssorient, (unsigned long) printvertex, printvertex[0],
				printvertex[1]);
	sdest(*s, printvertex);
	if (printvertex == (vertex) NULL)
		printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
	else
		printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n", 3 - s->ssorient, (unsigned long) printvertex, printvertex[0],
				printvertex[1]);
...
#else /* not SINGLE */
	_control87(_PC_53, _MCW_PC); /* Set FPU control word for float precision. */
#endif /* not SINGLE */
#endif /* CPU86 */
#ifdef LINUX
#ifdef SINGLE
...
#else /* not SINGLE */
	/*  cword = 4735; */
	cword = 4722; /* set FPU control word for float precision */
#endif /* not SINGLE */
	_FPU_SETCW(cword);
#endif /* LINUX */
...
//#else /* not ANSI_DECLARATORS */
//float counterclockwiseadapt(pa, pb, pc, detsum)
//vertex pa;
//vertex pb;
//vertex pc;
//float detsum;
...
//#else /* not ANSI_DECLARATORS */
//float counterclockwise(m, b, pa, pb, pc)
//struct mesh *m;
//struct behavior *b;
//vertex pa;
//vertex pb;
...
//#else /* not ANSI_DECLARATORS */
//float incircleadapt(pa, pb, pc, pd, permanent)
//vertex pa;
//vertex pb;
//vertex pc;
//vertex pd;
...
//#else /* not ANSI_DECLARATORS */
//float incircle(m, b, pa, pb, pc, pd)
//struct mesh *m;
//struct behavior *b;
//vertex pa;
//vertex pb;
...
//#ifdef ANSI_DECLARATORS
float orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd, float aheight, float bheight, float cheight,
		float dheight, float permanent)
//#else /* not ANSI_DECLARATORS */
//float orient3dadapt(pa, pb, pc, pd,
...
{
	INEXACT float adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
	float det, errbound;
...
//#ifdef ANSI_DECLARATORS
float orient3d(struct mesh *m, struct behavior *b, vertex pa, vertex pb, vertex pc, vertex pd, float aheight,
		float bheight, float cheight, float dheight)
//#else /* not ANSI_DECLARATORS */
//float orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
//struct mesh *m;
//struct behavior *b;
//vertex pa;
//vertex pb;
...
//#else /* not ANSI_DECLARATORS */
//float nonregular(m, b, pa, pb, pc, pd)
//struct mesh *m;
//struct behavior *b;
//vertex pa;
//vertex pb;
...
//#ifdef ANSI_DECLARATORS
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri, float minedge, vertex enqapex,
		vertex enqorg, vertex enqdest)
//#else /* not ANSI_DECLARATORS */
...

{
	...
	while (totalsamplesleft > 0)
	{
		...
		{
			sampletri.tri =
					(triangle *) (firsttri + (randomnation((unsigned int) population) * m->triangles.itembytes));
			...
			if (brokensubseg.ss != m->dummysub)
			{
				...
				if (segmentflaws)
				{
					...
					if (enq)
					{
						...
						{
							printf("  Queueing encroached subsegment (%.12g, %.12g) (%.12g, %.12g).\n",
									encroached->subsegorg[0], encroached->subsegorg[1], encroached->subsegdest[0],
...

{
	...
	*eventheap = (struct event **) trimalloc(maxevents * (int) sizeof(struct event *));
...
//#ifdef ANSI_DECLARATORS
float circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, float ccwabc)
//#else /* not ANSI_DECLARATORS */
//float circletop(m, pa, pb, pc, ccwabc)
//struct mesh *m;
//vertex pa;
//vertex pb;
//vertex pc;
...

{
	...
	poolinit(&m->splaynodes, sizeof(struct splaynode), SPLAYNODEPERBLOCK, SPLAYNODEPERBLOCK, 0);
	...
	while (heapsize > 0)
	{
		...
		if (nextevent->xkey < m->xmin)
		{
			...
			if (randomnation(SAMPLERATE) == 0)
			{
				...
				splayroot = circletopinsert(m, b, splayroot, &lefttri, leftvertex, midvertex, rightvertex,
						nextevent->ykey);
			}
		}
		else
...
//
//#ifdef ANSI_DECLARATORS
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist, float *triangleattriblist,
		float *trianglearealist, int elements, int corners, int attribs, int *segmentlist, int *segmentmarkerlist,
...

{
	...
	while (triangleloop.tri != (triangle *) NULL)
	{
		...
		for (j = 0; j < m->eextras; j++)
		{
			...
			else
			{
				setelemattribute(triangleloop, j,
						(float) strtod(stringptr, &stringptr));
			}
#endif /* not TRILIBRARY */
		}
		...
		if (b->vararea)
		{
			...
			{
				area = (float) strtod(stringptr, &stringptr);
			}
#endif /* not TRILIBRARY */
...

{
	...
	success = insertvertex(m, b, newvertex, &searchtri1, (struct osub *) NULL, 0, 0);
...

{
	...
	if (b->verbose > 1)
	{
		printf("  Connecting (%.12g, %.12g) to (%.12g, %.12g).\n", endpoint1[0], endpoint1[1], endpoint2[0],
				endpoint2[1]);
	}
				...
				if (neighborsubseg.ss == m->dummysub)
				{
					...
					if (b->verbose > 2)
					{
						...
						printf("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", deadorg[0], deadorg[1],
								deaddest[0], deaddest[1], deadapex[0], deadapex[1]);
					}
					infect(neighbor);
...
#ifdef SINGLE
	printf("Alternatively, try recompiling me with float precision\n");
	...
	while ((m->badsubsegs.items > 0) && (m->steinerleft != 0))
	{
		...
		while ((encloop != (struct badsubseg *) NULL) && (m->steinerleft != 0))
		{
			...
			if (!deadsubseg(currentenc.ss) && (eorg == encloop->subsegorg) && (edest == encloop->subsegdest))
			{
				...
				{
					segmentlength = sqrt(
							(edest[0] - eorg[0]) * (edest[0] - eorg[0]) + (edest[1] - eorg[1]) * (edest[1] - eorg[1]));
				...
				if (!b->noexact)
				{
					...
					divisor =
							((eorg[0] - edest[0]) * (eorg[0] - edest[0]) + (eorg[1] - edest[1]) * (eorg[1] - edest[1]));
				...
				{
					printf("  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).\n", eorg[0],
							eorg[1], edest[0], edest[1], newvertex[0], newvertex[1]);
				}
				...
				success = insertvertex(m, b, newvertex, &enctri, &currentenc, 1, triflaws);
...

{
			...
			&& (bapex == badtri->triangapex))
	{
		...
		else
		{
			...
			success = insertvertex(m, b, newvertex, &badotri, (struct osub *) NULL, 1, 1);
			if (success == SUCCESSFULVERTEX)
			{
				if (m->steinerleft > 0)
				{
...

{
	...
	poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK, BADSUBSEGPERBLOCK, 0);
	if (b->verbose)
	{
	...
	if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest)
	{
		...
		poolinit(&m->badtriangles, sizeof(struct badtriang), BADTRIPERBLOCK, BADTRIPERBLOCK, 0);
		...
		poolinit(&m->flipstackers, sizeof(struct flipstacker), FLIPSTACKERPERBLOCK, FLIPSTACKERPERBLOCK, 0);
		m->checkquality = 1;
		if (b->verbose)
		{
...

{
	...
	for (i = 0; i < m->invertices; i++)
	{
		...
		x = (float) strtod(stringptr, &stringptr);
		stringptr = findfield(stringptr);
		if (*stringptr == '\0')
		{
		...
		y = (float) strtod(stringptr, &stringptr);
		vertexloop[0] = x;
		vertexloop[1] = y;
		...
		for (j = 2; j < 2 + m->nextras; j++)
		{
			...
			{
				vertexloop[j] = (float) strtod(stringptr, &stringptr);
			}
		}
		if (nodemarkers)
		{
...
//#ifdef ANSI_DECLARATORS
void transfernodes(struct mesh *m, struct behavior *b, float *pointlist, float *pointattriblist, int *pointmarkerlist,
...
void readholes(struct mesh *m, struct behavior *b,
		FILE *polyfile, char *polyfilename, float **hlist, int *holes,
		float **rlist, int *regions)
//#else /* not ANSI_DECLARATORS */
...

{
	...
	*holes = (int) strtol(stringptr, &stringptr, 0);
	if (*holes > 0)
	{
		holelist = (float *) trimalloc(2 * *holes * (int) sizeof(float));
		*hlist = holelist;
		for (i = 0; i < 2 * *holes; i += 2)
		{
			stringptr = readline(inputline, polyfile, polyfilename);
			...
			{
				holelist[i] = (float) strtod(stringptr, &stringptr);
			}
			stringptr = findfield(stringptr);
			if (*stringptr == '\0')
			{
			...
			{
				holelist[i + 1] = (float) strtod(stringptr, &stringptr);
			}
		}
	}
	else
	...
	if ((b->regionattrib || b->vararea) && !b->refine)
	{
		...
		*regions = (int) strtol(stringptr, &stringptr, 0);
		if (*regions > 0)
		{
			regionlist = (float *) trimalloc(4 * *regions * (int) sizeof(float));
			*rlist = regionlist;
			index = 0;
			for (i = 0; i < *regions; i++)
			{
				...
				{
					regionlist[index++] = (float) strtod(stringptr, &stringptr);
				}
				stringptr = findfield(stringptr);
				if (*stringptr == '\0')
				{
...

{
	...
	if (*trianglelist == (int *) NULL)
	{
		*trianglelist = (int *) trimalloc(
				(int) (m->triangles.items * ((b->order + 1) * (b->order + 2) / 2) * sizeof(int)));
	}
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
		float *holelist, int holes, float *regionlist, int regions,
		int argc, char **argv)
//#else /* not ANSI_DECLARATORS */
...
//#ifdef ANSI_DECLARATORS
void writevoronoi(struct mesh *m, struct behavior *b, float **vpointlist, float **vpointattriblist,
		int **vpointmarkerlist, int **vedgelist, int **vedgemarkerlist, float **vnormlist)
//#else /* not ANSI_DECLARATORS */
...

{
	...
	if (b.refine)
	{
		...
		m.hullsize = reconstruct(&m, &b, in->trianglelist, in->triangleattributelist, in->trianglearealist,
				in->numberoftriangles, in->numberofcorners, in->numberoftriangleattributes, in->segmentlist,
