...
public:
	...
	NodeState(SceneNode* node, const Matrix3x4& matrix,
...
public:
	 ...
	 * @return 成功为true
	 */
	static bool FlatModel(View* view, vector<ShapeNode *>& shapeList, int row,
			int column);
...
private:
	...
	static void AwayFromCenterCallback(void* data, SceneNode* node);

	void AwayFromCenter(const Vector3& topCenter, GroupNode* subNode);

	void CacheMatrixState();

	static void CacheMatrixState(void* data, SceneNode* node);
	 ...
	 * @return
	 */
	bool GetMatrix(SceneNode* nodeAddr, NodeState& nodeState);
	/**
	 * 将矩阵加入缓存中
	 * @param nodeAddr
	 * @param matrix
	 */
	void AddMatrix(SceneNode* nodeAddr, const NodeState& nodeState);
...
private:
	...
	map<SceneNode*, NodeState> m_allNodeMatrixsCache;
