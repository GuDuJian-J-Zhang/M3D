...
namespace M3D
{
	void PerspectiveOperator::Show(SVIEW::View* pView, int viewType,
		bool allowRotate, bool allowTran, bool allowScale,bool useAni)
	{
		Apply(pView, viewType, allowRotate, allowTran, allowScale, useAni && m_allowUseAni);
	}
		...
		bool allowRotate, bool allowTran, bool allowScale)
	{
		...
		*m_animationTo = to;
		...
		m_animationdistimvMatrix->rotation = camera->GetOldRotation();
		m_animationdistimvMatrix->moveVector = camera->GetOldPosition();
		m_animationdistimvMatrix->scaleFactor = camera->GetZoom();
	...
	{
		if (this->m_pView->GetSceneManager()->GetModel())
		{
			//this->m_pView->Lock();
			this->m_pView->GetSceneManager()->Lock();
			//m_pSelectedNodes->SetRotationAroundCenter(rotation);

			Quaternion deltaRotation = rotation.Inverse();
			deltaRotation = this->m_pView->GetSceneManager()->GetCamera()->GetRotation().Inverse()* deltaRotation;
			this->m_pView->GetSceneManager()->GetCamera()->RotateAroundCenter(deltaRotation, TS_WORLD);

			this->m_pView->GetSceneManager()->UnLock();
			//this->m_pView->UnLock();
		}
	...
	void PerspectiveOperator::ApplyMovementToSelectedNode(const Vector3& mov)
	{
		if (this->m_pView->GetSceneManager()->GetModel())
		{
			...
			this->m_pView->GetSceneManager()->GetCamera()->SetWorldPosition(mov);
			this->m_pView->GetSceneManager()->UnLock();
		}
	}
	...
	void PerspectiveOperator::ApplyScaleToSelectedNode(float & scale)
	{
		if (this->m_pView->GetSceneManager()->GetModel())
		{
			...
			this->m_pView->GetSceneManager()->GetCamera()->SetZoom(scale);
			this->m_pView->GetSceneManager()->UnLock();
		}
	}
	...
	bool PerspectiveOperator::StartAnimation()
	{
		...
		{
			if (this->m_bAllowRotate)
			{
				this->ApplyRotationToSelectedNode(to.rotation);
			}
			if (this->m_bAllowScale)
			{
				this->ApplyScaleToSelectedNode(to.scaleFactor);
			}
			if (this->m_bAllowTran)
			{
				this->ApplyMovementToSelectedNode(to.moveVector);
			}
			this->m_bAllowRotate = true;
		}
	...
	void* PerspectiveOperator::commonTimerTask(void* arg)
	{
			...
			if (perspectiveOperator->m_pView->GetSceneManager()->GetModel() != NULL)
			{
