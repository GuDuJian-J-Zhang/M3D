...
void LogMemoryState(char* appendInfo) {
	if (appendInfo == NULL) {
		...
		LOGD(
				(strAppendInfo + "当前内存:" + MemoryHelper::Instance()->ProcessMemoryInfo()).c_str());
	}
}
...
void SVL2AsynReader::ReadFromBuffer(char* fileBuffer, int bufferLength) {
	...
	if (this->m_fileBufferLength > 0) {
		Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) this->m_svl2Doc;
		if (m_svl2Doc->isNotNull()) { //打开文件
			(*m_svl2Doc)->OpenFile(this->m_fileBufferLength,
					this->m_fileBuffer);
		}
	}
}
...
void SVL2AsynReader::FillModelColor(Model* model) {
	...
	if (fileInfo) {
		if (/*true||*/fileInfo->GetInstanceMaterialId() > 0) {
			...
			this->GetMatrialColor(fileInfo->GetInstanceMaterialId(),
					modelInitColor, modelMateril);
			if (modelMateril) {
				model->SetMaterial(modelMateril);
				//LOGE("Model has Material,Please!!%s",model->GetName().c_str());
			}
			if (modelInitColor.m_r >= 0.0f && modelInitColor.m_g >= 0.0f
...
void SVL2AsynReader::FillModelMesh(View* view, Model* model) {
	...
	if (fileInfo) {
		...
		Stk_ModelPtr stkModel = (*m_svl2Doc)->GetModelByID(
				stk_nodeId - m_SVLIDOffset);
		if (stkModel.isNotNull()) {
			Model* pModel = FindPrototypeFromCache(stkModel->GetID());
			if (pModel != NULL) {
				CopyFormPrototype(static_cast<Model*>(model),
...
int SVL2AsynReader::GetSVLXFileItem(const std::string& i_strFileName,
...
int SVL2AsynReader::GetSVLXFileItemToFile(const std::string& i_strFileName,
		const std::string& i_strOutFilePath, bool bByExt) {
	Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) this->m_svl2Doc;
	return (*m_svl2Doc)->GetSVLXFileItemToFile(i_strFileName, i_strOutFilePath,
			bByExt);
}
int SVL2AsynReader::GetSVLXFileItemToFileByReOpen(string & path,
...
void SVL2AsynReader::FillModelDrawDataMergeFace(Model* model,
		void* pstk_NodePtr) {
	...
	if (stkPart.isNotNull()) {
		...
		if (meshCount) {
			...
			std::vector<M3D_INDEX_TYPE>* indexArray =
					bodyDrawData->GetIndexArray();
			std::vector<Vector3>* textureCoords =
					bodyDrawData->GetTextureCoordArray();
			...
			for (int i = 0; i < meshCount; i++) {
				...
				for (int pntIndex = 0; pntIndex < vecVert.size() / 3;
						pntIndex++) {
				...
				std::map<STK_INT32, Stk_MeshFacePtr>& meshFaces =
						meshPtr->GetMeshFaces();
				std::map<STK_INT32, Stk_MeshFacePtr>::iterator meshIt =
						meshFaces.begin();

				//获取面的个数
				int numMeshFace = meshPtr->GetNbMeshFace();
				for (; meshIt != meshFaces.end(); ++meshIt) { //纪录前一次已经存储的数据大小
					...
					if (vecTrians.size() > 0) {
						...
						for (int ii = 0; ii < vecTrians.size(); ii++) {
							unsigned int tmpIndex = vecTrians[ii]
									+ bodyMeshHasDataOffset;
...
void SVL2AsynReader::FillModelDrawDataMergeFaceWithColor(Model* model,
		void* pstk_NodePtr) {
	Stk_ModelPtr stkPart = *(Stk_ModelPtr*) pstk_NodePtr;
	if (stkPart.isNotNull()) {
		int meshCount = 1;
		if (meshCount) {
			...
			for (int i = 0; i < meshCount; i++) {
				...
				vector<unsigned int> vecTrians;
				//获取面的个数
				std::map<STK_INT32, Stk_MeshFacePtr>& meshFacesMap =
						meshPtr->GetMeshFaces();
				std::map<STK_INT32, Stk_MeshFacePtr>::iterator facesIt =
						meshFacesMap.begin();
				...
				for (; facesIt != meshFacesMap.end(); facesIt++) {
					...
					meshFacePtr->GetIndex(vecTrians);
					if (vecTrians.size() > 0) {
						if (priFaceMaterialIndex
								!= meshFacePtr->GetMaterialID()) {
							...
							Color lfaceColor;
							BaseMaterial* faceMaterial = NULL;
							this->GetMatrialColor(meshFacePtr->GetMaterialID(),
									lfaceColor, faceMaterial);
							lface->SetInitColor(lfaceColor);
							if (faceMaterial != NULL) {
								lface->SetMaterial(faceMaterial);
							}
						...
						facemeshData->SetDataLength(
								indexArray->size() - faceOffset);
					}
				}
...
void SVL2AsynReader::FillLineSetDrawData(Model* i_model, void* i_stkModelPtr) {
	...
	for (it = mapStkLinsetPtr.begin(); it != mapStkLinsetPtr.end(); it++) {
		...
		vector<Stk_CurveEntityPtr> vecCurveEntity =
				stkLineSetPtr->GetAllLines();
		...
		for (int lineIndex = 0; lineIndex < vecCurveEntity.size();
				lineIndex++) {
...
bool SVL2AsynReader::FillCurve(Stk_CurveEntityPtr curCurve,
		RefPolyLine * edgeLine, SPolyLine * polyLine) {
	bool ret = false;
	...
	case CURVE_TYPE_LINE: {
		...
		if (dataLength >= 2) {
			...
			//加入中间部分点
			//在加入最后一个点
			polyLine->AddPoints(Vector3(endPnt.X, endPnt.Y, endPnt.Z));
			ret = true;
		}
		break;
	}
	...
	if (stkPart.isNotNull()) {
		...
		if (meshCount) {
			...
			for (int i = 0; i < meshCount; i++) {
				...
				std::vector<Vector3>* bodyPnts =
						bodyDrawData->GetPositionArray();
				std::vector<Vector3>* bodyNormals =
						bodyDrawData->GetNormalArray();
				std::vector<M3D_INDEX_TYPE>* indexArray =
						bodyDrawData->GetIndexArray();
				std::vector<Vector3>* textureCoords =
						bodyDrawData->GetTextureCoordArray();
				...
				for (; facesIt != meshFacesMap.end(); facesIt++) {
					...
					if (vecTrians.size() > 0) {
						...
						//lface->SetColor(*firstFace->GetInitColor());
						if (useIndex) { //lface->SetColor(*firstFace->GetInitColor());
							Mesh* facemeshData = new Mesh(bodyDrawData);
							//纪录前一次已经存储的数据大小
							faceOffset = indexArray->size();
							facemeshData->SetDataOffset(faceOffset);
	...
	if (SVIEW::Parameters::Instance()->GetLoadExternInfo()) {
		...
		*m_svl2Doc = Stk_DocumentPtr::CreateObject();
		if (SVIEW::Parameters::Instance()->m_IsUseGeoData) {
			...
			if (meshFaceAtrrPtr) {
				m_view->GetSceneManager()->GetExtendInfoManager()->SetAllMeshEdgeAttr(
						*meshEdgeAtrrPtr);
			}
...
void SVL2AsynReader::AddGeoAttribute(Model* model, string & path,
		View *m_view) {
	if (SVIEW::Parameters::Instance()->GetLoadExternInfo()) {
		wstring wfilePath = Platform::StringToWString(path, "auto");
			...
			if (*it) {
				...
				for (int i = 0; i < faceList.size(); i++) {
					...
					if (face && face->GetData()) {
						...
						GetTriMeshGeoAttribute(model->GetProtoTypeId(),
								facemeshData, meshFaceID, svlDoc);
					}
				}
				...
				for (int i = 0; i < EdgeList->size(); i++) {
					...
					if (edge && edge->GetLineData()) {
						...
						GetEdgeGeoAttribute(model->GetProtoTypeId(), edgeLine,
								meshEdgeID, svlDoc);
					}
				}
			}
...
void SVL2AsynReader::AddEdgeDataToBody(Body* body, void* pStkMesh,
		int readLodLevel) {
	...
	if (stkEdgeCount) {
		...
		std::map<STK_INT32, Stk_MeshEdgePtr>& edgesMap =
				meshPtr->GetMeshEdges();
		std::map<STK_INT32, Stk_MeshEdgePtr>::iterator edgeIt =
				edgesMap.begin();
...
void SVL2AsynReader::AddMergeEdgeDataToBody(Body* body, void* pStkMesh,
		int readLodLevel) {
...
bool SVL2AsynReader::FillMergeEdges(void* stk_edge, vector<float>* outVertices,
		SPolyLine* bodyPolyLine, RefPolyLine * edgeLine) {
...
bool SVL2AsynReader::FillEdges(void* stk_edge, vector<float>* outVertices,
		SPolyLine* bodyPolyLine, RefPolyLine * edgeLine) {
	Stk_MeshEdgePtr meshEdge = *((Stk_MeshEdgePtr*) stk_edge);
	vector<STK_UINT32> edgeIndex;
	meshEdge->GetIndex(edgeIndex);
	...
	void SVL2AsynReader::FillAssembly(Model* model, void* instancePtr)
	{
		...
		if (modelID > 0) {
			...
			m_modelStkModelMap.insert(
					Model_StkModel_Map::value_type(model,
							modelID + m_SVLIDOffset));
		}

		vector<Stk_InstancePtr>& vecChildIns = instance->getChildren();
		for (int subModelIndex = 0; subModelIndex < vecChildIns.size();
				subModelIndex++) {
...
string SVL2AsynReader::GetInstanceName(Model* model) {
	string insName = "";
	map<STK_UINT32, std::wstring>::iterator it =
			this->i_mapInstanceIDtoName.find(
					model->GetSVLId() - this->m_SVLIDOffset);
...
Model* SVL2AsynReader::ReadFile(const char* thePath) {
	...
	*m_svl2Doc = Stk_DocumentPtr::CreateObject();
	...
	string effectName = Platform::WStringToString(
			(*m_svl2Doc)->GetGlobalEffect());
	...
	for (; it != parametersMap.end(); it++) {
		...
		if (type == "Texture2D") {
			...
			if (buffersize > 0 && data) {
				...
				Texture2D* texture2d =
						(Texture2D*) this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateTexture(
								savePath, Texture::TEXTURE_2D);
			...
			for (int i = 0; i < imageIDStrArray.size(); i++) {
				...
				Image* image =
						this->m_view->GetSceneManager()->GetResourceManager()->GetImage(
								savePath);
				if (!image) {
					image = new Image();
					image->SetPath(savePath);
			...
			TextureCube * cubeTexture =
					(TextureCube*) this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateCubeMappingTexture(
							savePathes, cubeImages);
			...
			if (!material->SetUniformParameter(name, uniform)) {
				...
				Uniform* oldUniform = material->GetUniformParameter(name);
				TextureCube* oldValue = anyCast<TextureCube*>(
						oldUniform->value);
...
void SVL2AsynReader::GetImageData(unsigned int imageID,
		unsigned int & o_buffersize, char * &o_data, string& o_svePath) {
	Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) this->m_svl2Doc;
	...
	(*m_svl2Doc)->GetSVLXFileItem(Platform::WStringToString(strPicPath),
			o_buffersize, &o_data, false);
}

bool SVL2AsynReader::GetMatrial(int materialId, Color& difuseColor,
		BaseMaterial* &material) {
	material = NULL;
	...
	if (materialPtr.isNotNull()) {
		map<int, BaseMaterial*>::iterator it = m_protoTypeMaterialCache.find(
				materialId);
		if (it != m_protoTypeMaterialCache.end()) {
			material = it->second;
		}

		map<int, Color>::iterator itColor = m_protoTypeColorCache.find(
				materialId);
		...
		if (materialType == StkMaterialTypeEnum::STK_MATERIALTYPE_SIMPLAE) {
			...
			HoteamSoft::SVLLib::Stk_TexturePtr pStkTexture =
					materialPtr->GetDiffuseTexture();
			...
			if (pStkTexture.isNotNull() || textures.size() > 0) {
				...
				if (material) {
					if (pStkTexture.isNotNull()) {
						...
						m_protoTypeMaterialCache.insert(
								std::pair<int, BaseMaterial*>(materialId,
										material));
					...
					if (textures.size() > 0) {
						...
						for (int i = 0; i < textures.size(); i++) {
							HoteamSoft::SVLLib::Stk_TexturePtr pTexture =
									textures[i];
							if (pTexture.isNotNull()) {
								HoteamSoft::SVLLib::Stk_ImagePtr pImage =
										pTexture->GetTextureImage();

								string strPicPath = Platform::WStringToString(
										pImage->GetPictureSavePath());
								...
								Stk_DocumentPtr* m_svl2Doc =
										(Stk_DocumentPtr*) this->m_svl2Doc;
								name += strPicPath;
								(*m_svl2Doc)->GetSVLXFileItem(strPicPath,
										buffersize, &data, false);
								//将数据赋值给图片对象
						...
						Texture * cubeTexture =
								this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateCubeMappingTexture(
										name, cubeImages);

						((Material*) material)->SetReflectiveTexture(
								cubeTexture);
					}
				}
			} else {
...
bool SVL2AsynReader::GetMatrialColor(int materialId, Color& difuseColor,
		BaseMaterial* &material) {
	material = NULL;
	difuseColor = Color::GRAY;
	...
	if (materialPtr.isNotNull()) {
		...
		if (materialType == StkMaterialTypeEnum::STK_MATERIALTYPE_SIMPLAE
			...
			material =
					this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateMaterial(
							materialIDStr, MaterialType_Phong);
			//获取RGB的值
			materialPtr->GetDiffuseColor(&(difuseColor.m_r));
			materialPtr->GetTransparency(difuseColor.m_a);
		...
		} else if (materialType == StkMaterialTypeEnum::STK_MATERIALTYPE_PHONG
				|| materialType
						== StkMaterialTypeEnum::STK_MATERIALTYPE_SIMPLAE) {
			material =
					this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateMaterial(
							materialIDStr, MaterialType_Phong);
			...
			if (tempColor.m_r >= 0 && tempColor.m_g >= 0
					&& tempColor.m_b >= 0) {
			...
			//获取diffuse
			HoteamSoft::SVLLib::Stk_TexturePtr diffuseTexture =
					materialPtr->GetDiffuseTexture();
			if (diffuseTexture.isNotNull()) {
				...
				hasTexture = true;
				//如果u v 不同时为1
				if (Abs(uScale - 1) < 0.001 && Abs(vScale - 1.0f) < 0.001
						|| Abs(uScale) < 0.001f || Abs(vScale) < 0.001f) {
				} else {
					//构造变换纹理坐标变换矩阵
			...
			HoteamSoft::SVLLib::Stk_TexturePtr matcapTexture =
					materialPtr->GetMatcapMap();
			...
			//获取normalMap
			{
				HoteamSoft::SVLLib::Stk_TexturePtr normalTexture =
						materialPtr->GetNormalMap();
				if (normalTexture.isNotNull()) {
					Texture2D * normalMapping =
							static_cast<Texture2D*>(GetTexture2D(&normalTexture));
			...
			m_protoTypeMaterialCache.insert(
					std::pair<int, BaseMaterial*>(materialId, material));
			m_protoTypeColorCache.insert(
					std::pair<int, Color>(materialId, difuseColor));
		} else if (materialType == MaterialType_MatCap) {
			material =
					this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateMaterial(
							materialIDStr, MaterialType_MatCap);
			MatCapMaterial* tempMaterial =
					static_cast<MatCapMaterial*>(material);
			wstring wMaterialName = materialPtr->GetMaterialName();
			string materialName = Platform::WStringToString(wMaterialName,
					"auto");
			tempMaterial->DisplayName(materialName);
			...
			material =
					this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateMaterial(
							materialIDStr, MaterialType_Pbr);
			...
			//TODO
			{
				...
				HoteamSoft::SVLLib::Stk_TexturePtr diffuseTexture =
						materialPtr->GetDiffuseTexture();
				if (diffuseTexture.isNotNull()) {
					Texture2D * diffuseMapping =
							static_cast<Texture2D*>(GetTexture2D(
									&diffuseTexture));
            ...
            if (!material || !material->GetNeedsUpdateUniformParamerers()) {
                return false;
            }
            ...
            material->SetUniformParameter("type", uniform);
            ParseMaterialParameters(materialPtr, material);
            LOGE("材质名称：%s",material->GetName().c_str());
		...
		material =
				this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateMaterial(
						M3D_DEFAULT_MATERIAL, MaterialType_Phong);
...
Texture * SVL2AsynReader::GetTexture2D(void* stkTexture) {
			...
			*(Stk_TexturePtr*) stkTexture;
	{
		...
		string imagePathWithOffset = Platform::WStringToString(strPicPath)
				+ "||" + StringHelper::IntToString(this->m_SVLIDOffset);
		Image* image =
				this->m_view->GetSceneManager()->GetResourceManager()->GetImage(
						imagePathWithOffset);
		if (image) {
			Texture2D* texture2d =
					(Texture2D*) this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateTexture(
							imagePathWithOffset, Texture::TEXTURE_2D);
			texture2d->SetImage(image);
			ret = texture2d;
		} else {
			Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) this->m_svl2Doc;
			(*m_svl2Doc)->GetSVLXFileItem(Platform::WStringToString(strPicPath),
					o_nudataSize, &o_chDataP, false);
			if (o_nudataSize <= 0) {
			...
			if (o_nudataSize > 0 && o_chDataP) {
				...
				if (!image) {
					...
					AddRefMe(image);
					texture2d->SetImage(image);
					this->m_view->GetSceneManager()->GetResourceManager()->AddImage(
							imagePathWithOffset, image);
					ReleaseMe(image);
				}
				ret = texture2d;
			}
...
void SVL2AsynReader::LoadFileInfo() {
	...
	Stk_LoadInf* infoFilePtr =
			(*m_svl2Doc)->GetLoadInf()/*Stk_LoadInf::GetInstance()*/;
	...
	if (type == 1) //颜色
		...
		if (style == 1) {
			...
			for (map<string, string>::iterator it = items.begin();
					it != items.end(); it++) {
				string name = it->first;
				...
				Color top(StringHelper::StringToFloat(tops[0]),
						StringHelper::StringToFloat(tops[1]),
						StringHelper::StringToFloat(tops[2]));
				vector<string> bottoms = StringHelper::Split(colors[1], " ");
				Color bottom(StringHelper::StringToFloat(bottoms[0]),
						StringHelper::StringToFloat(bottoms[1]),
						StringHelper::StringToFloat(bottoms[2]));
		...
		if (style == 1) {
			...
			for (map<string, string>::iterator it = items.begin();
				...
				Texture2D* texture2d =
						(Texture2D*) this->m_view->GetSceneManager()->GetResourceManager()->GetOrCreateTexture(
								val, Texture::TEXTURE_2D);
				texture2d->SetImageParameter(TEXTURE_LOAD_RGBA,
						TEXTURE_FLAG_MIPMAPS | TEXTURE_FLAG_INVERT_Y);
		...
		if (style == 1) {
			...
			for (map<string, string>::iterator it = items.begin();
				...
				for (int i = 0; i < pathes.size(); i++) {
					...
					(*m_svl2Doc)->GetSVLXFileItem(imagePath, buffersize, &data,
							false);
					Image* image =
							this->m_view->GetSceneManager()->GetResourceManager()->GetImage(
									imagePath);
		...
		if (infoSize > 0) {
			...
			if (realSrc.length() > 0) {
				...
				if (realSrc.find(tab) != string::npos) {
					realSrc.erase(realSrc.find(tab.c_str()) + tab.length(),
							realSrc.length());
				} else {
					tab = "}}}}";
					if (realSrc.find(tab) != string::npos) {
						...
						if (realSrc.find(tab) != string::npos) {
							realSrc.erase(
									realSrc.find(tab.c_str()) + tab.length(),
									realSrc.length());
						}
					}
				}
			...
			if (rootIt != infoDoc.MemberEnd() && rootIt->value.IsObject()) {
				...
				if (sceneIt != infoNode.MemberEnd()
						&& sceneIt->value.IsObject()) {
					...
					if (sceneNode.HasMember("lights")) {
						...
						for (int i = 0; i < lightsSize; i++) {
							...
							if (light.HasMember("position")) {
								string positionStr =
										light["position"].GetString();
								vector<string> strarray = StringHelper::Split(
										positionStr, ",");
								m3dlight->SetPosition(
										Vector3(
												StringHelper::StringToFloat(
														strarray[0]),
												StringHelper::StringToFloat(
														strarray[1]),
												StringHelper::StringToFloat(
							...
							if (light.HasMember("spotDirection")) {
								string directionStr =
										light["spotDirection"].GetString();
								vector<string> strarray = StringHelper::Split(
										directionStr, ",");
								m3dlight->SetSpotDirection(
										Vector3(
												StringHelper::StringToFloat(
														strarray[0]),
												StringHelper::StringToFloat(
							...
							if (light.HasMember("specular")) {
								string specularStr =
										light["specular"].GetString();
								vector<string> strarray = StringHelper::Split(
										specularStr, ",");
								m3dlight->SetSpecular(
										Color(
												StringHelper::StringToFloat(
														strarray[0]),
												StringHelper::StringToFloat(
					...
					//读取viewmode
					Value::MemberIterator viewmoderIt = sceneNode.FindMember(
							"viewmode");
					if (viewmoderIt != sceneNode.MemberEnd()
							&& viewmoderIt->value.IsObject()) {
						Value& viewmodeNode = viewmoderIt->value;
						if (viewmodeNode.HasMember("operationType")) {
							int type = viewmodeNode["operationType"].GetInt();
							switch (type) {
							case 1:
								this->m_view->setWalkThrough(
										HANDLER_WALKTHROUGH);
								break;
							case 2:
						...
						if (viewmodeNode.HasMember("upDirection")) {
							int direction =
									viewmodeNode["upDirection"].GetInt();
							Vector3 upDirection;
							switch (direction) {
							case 1:
							...
							if (m_view) {
								...
								touchHandler->SetUpDirection(upDirection,
										m_view);
...
void SVL2AsynReader::updateReadInstancePercent() {
	...
	if (this->m_instanceCount) {
		this->SetPercent(
				0.5f + 0.5f * readInstanceIndex * 1.0f / this->m_instanceCount);
	} else {
		this->SetPercent(0);
	}
}
...
string SVL2AsynReader::GetSVLMaterialID(Model* model, unsigned int renderID) {
	...
	if (renderID >= 0) {
		materialID = "SVLXRender" + StringHelper::IntToString(model->GetID())
		...
		if (stkModel.isNotNull()) {
			...
			if (!IsNaN(boxData[0]) && Abs(boxData[0]) < checkLimint
				...
				BoundingBox modelBox = BoundingBox(
						Vector3(boxData[0], boxData[1], boxData[2]),
						Vector3(boxData[3], boxData[4], boxData[5]));
...
void SVL2AsynReader::GetInstanceData(Model* model, void* instancePtr) {
	...
	Stk_InstancePtr instance =
			instancePtr != NULL ? *(Stk_InstancePtr*) instancePtr : NULL;
...
//                }
		...
		if (pOutPMIData.size() > 0) {
			...
			for (int i = 0; i < pOutPMIData.size(); i++) {
				...
				pmis.insert(
						map<int, PMIData*>::value_type(pTmpPMI->GetID(),
								pTmpPMI));
			}
			model->SetModelExtInfo(
					m_view->GetSceneManager()->GetExtendInfoManager());
			model->SetPMIs(pmis);
		}
	}
...
bool SVL2AsynReader::GetPMIInfo(vector<Stk_PMIPtr>* pStkPMIList, int *outPMINum,
	...
	for (int i = 0; i < pStkPMIList->size(); i++) {
		...
		//		pnt.clear();
		//		Indices.clear();
		HoteamSoft::SVLLib::StkTermTypeEnum nEndSymbol =
		...
		if (ParallelScreenflg) {
			...
			if (pLeads.size() > 0) {
				...
				pCurve =
						CurveList.size() > 0 ?
								CurveList[CurveList.size() - 1] : NULL;
			...
			for (u = 0; u < nComTextsNum; u++) {
				...
				if (8000 != pPMIData->m_Type) {
					...
					for (v = 0; v < nTextNum; v++) {
						...
						if (v == 0) {
							pPMIData->m_outramLocation = Vector3(Pos.X, Pos.Y,
									Pos.Z);
						}
						...
						//将aNember插入到mapLine
						mapLine.insert(
								pair<STK_INT32, vector<STK_INT32> >(nLin,
										aNember));
						nLin++; //行号？
						aNember.clear();
					}
				...
				if (nSize2 > 0) {
					...
					for (j = 0; j < nSize2; j++) {
						...
						string tmpStr = Platform::WStringToString(
								tmpText->GetTextContent());
...
void SVL2AsynReader::SetPolyLineForPMI(PMIData* pPMIData,
		vector<HoteamSoft::SVLLib::STK_PNT32>& vertexBuf,
...
ComText* SVL2AsynReader::ConvertStkComTextToComText(Stk_ComTextPtr stkComText) {
	...
	for (int j = 0; j < tmpTexts.size(); j++) {
		...
		string textString = Platform::WStringToString(
				tmpText->GetTextContent());
...
void SVL2AsynReader::GetTriMeshGeoAttribute(int iModelID,
		IVertexSet* pTriMeshData, int MeshID, void* svlDoc) {
...
GeometryAttribute* SVL2AsynReader::GetGeometryAttributeFromStk(
...
GeometryAttribute* SVL2AsynReader::GetPlanfaceGeoFromStk(
	...
	Stk_PlaneFaceAttributePtr stk_geoPlan =
			Stk_PlaneFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetRevolutionfaceGeoFromStk(
	...
	Stk_RevolutionFaceAttributePtr stk_geoRevo =
			Stk_RevolutionFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetCylinderfaceGeoFromStk(
	...
	Stk_CylinderFaceAttributePtr stk_geoRevo =
			Stk_CylinderFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetConicalfaceGeoFromStk(
	...
	Stk_ConeFaceAttributePtr stk_geoRevo =
			Stk_ConeFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetSpherefaceGeoFromStk(
	...
	Stk_SphereFaceAttributePtr stk_geoRevo =
			Stk_SphereFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetToroidaifaceFromStk(
	...
	Stk_ToroidalFaceAttributePtr stk_geoToroidail =
			Stk_ToroidalFaceAttributePtr::DynamicCast(stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetLineGeoFromStk(
	...
	Stk_LineAttributePtr stk_geoLine = Stk_LineAttributePtr::DynamicCast(
			stk_geo);
...
GeometryAttribute* SVL2AsynReader::GetEllipseGeoFromStk(
	...
	Stk_EllipseAttributePtr stk_geoEllipse =
			Stk_EllipseAttributePtr::DynamicCast(stk_geo);
...
void SVL2AsynReader::SVLXRenderToMaterial(BaseMaterial* material,
		void* stkTexture) {
	HoteamSoft::SVLLib::Stk_TexturePtr pStkTexture =
			*(Stk_TexturePtr*) stkTexture;
	{
...
void SVL2AsynReader::GetEdgeGeoAttribute(int iModelID, Curve* curve, int EdgeID,
		void* svlDoc) {
	Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) svlDoc;
	//TODO 合并分支
...
void SVL2AsynReader::ReadModelAttribute(Model* model) {
	if (this->m_view) {
		...
		for (int i = 0; i < allSubModels.size(); i++) {
			...
			if (this->m_useInstanceProperty
					&& subModel->GetSVLId() - this->m_SVLIDOffset > 0) {
				properties = svl2Doc->GetInstanceAllAttribute(
						subModel->GetInstatnceID());
			} else if (this->m_usePrototypeProperty
					&& subModel->GetProtoTypeId() > 0) {
				properties = svl2Doc->GetModelAllAttribute(
						subModel->GetProtoTypeId());
			}
...
void SVL2AsynReader::ReadUserData(Model* model) {
	if (this->m_view) {
		...
		//动画文件
		(*((Stk_DocumentPtr*) m_svl2Doc))->GetSVLXFileItem(".manual",
				m_dataSize, &m_dataP);
		...
		(*((Stk_DocumentPtr*) m_svl2Doc))->GetSVLXFileItem(".hotspot",
				h_dataSize, &h_dataP);
		...
		(*((Stk_DocumentPtr*) m_svl2Doc))->GetSVLXFileItem(".light", l_dataSize,
				&l_dataP);
...
//
//string SVL2AsynReader::GetViewInfoStr(Model* model, ModelView *modelView) {
//
//	READER_LOGI("Svlreader::GetViewInfoStr");
//	SModelFileInfo* fileInfo = model->GetFileInfo();
//	if (fileInfo) {
//		Stk_DocumentPtr* m_svl2Doc = (Stk_DocumentPtr*) this->m_svl2Doc;
//		int stk_nodeId = fileInfo->GetPartModelId();
...
//		model->SetModelExtInfo(
//				m_view->GetSceneManager()->GetExtendInfoManager());
//		for (Stk_ViewPtr view : stk_ViewList) {
//			if (view.operator ->()->GetID() == modelView->GetID() ) {
//				wstring value = pViewData;
//                break;
//			}
//		}
//	}
...
//获取视图信息
bool SVL2AsynReader::GetViewInfo(vector<Stk_ViewPtr> *pStkViewList,
		Model* inModel, int *outViewNum, vector<ModelView*>& outViewData) {
	...
	*outViewNum = 0;
	...
	for (int i = 0; i < ViewNum; i++) {
		...
		HoteamSoft::SVLLib::StkViewUsageEnum viewType =
				pViewData->GetUsageType();
		switch (viewType) {
		...
		if (viewIsActivated
				&& viewType != HoteamSoft::SVLLib::VIEW_USAGE_USER_CLIPPLANE) {
		...
		if (pCameraData != NULL) {
			...
			if (pCameraData->GetProjectType() == 1) {
				...
				cameraInfo.SetOrthoSize(
						Vector2(fHeight * pCameraData->GetAspectRatio(),
								fHeight));
			} else {
		...
		for (int j = 0; j < allPlaneList.size(); j++) {
			...
			Stk_ClipPlanePtr pPlaneSurfaceData = Stk_ClipPlanePtr::DynamicCast(
					allPlaneList[j]);
			...
			SectionPlane *plane = new SectionPlane(tmpOrigin, tmpNormal,
					tmpUDir, tmpVDir);
		...
		for (int k = 0; k < vecInsAttrPlcPath.size(); k++) {
			...
			if (HoteamSoft::SVLLib::STK_SUCCESS
					== pViewData->GetInsColorByPlcPath(wstrPlcPath, newColor)) {

				Color dispColor(newColor.Red, newColor.Green, newColor.Blue,
						newColor.Alpha);
			...
			if (HoteamSoft::SVLLib::STK_SUCCESS
					== pViewData->GetInsDisplayStateByPlcPath(wstrPlcPath,
							displayState)) {
			...
			pViewData->GetInsPlacementByPlcPath(wstrPlcPath, idPlacement,
					newMatrix);
		...
		map < HoteamSoft::SVLLib::StkEntityTypeEnum, vector
				< STK_ID >> mapViewPMI = pViewData->GetConnector();
		map<HoteamSoft::SVLLib::StkEntityTypeEnum, vector<STK_ID>>::iterator it =
				mapViewPMI.begin();
		for (; it != mapViewPMI.end(); it++) {
			...
			if (EntType == ENTITY_TYPE_PMI) {
				...
				for (vector<STK_ID>::iterator ite = vcEntityID.begin();
						ite != vcEntityID.end(); ite++) {
					pView->AddPMIId(*ite);
				}
