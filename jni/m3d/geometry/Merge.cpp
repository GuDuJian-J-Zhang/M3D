...
namespace M3D {
	...
	bool Merge::MergeModelFaces(Model* model, int MergeFlg, int iEliminateFlg, bool NeedSetMatrix)
	{
		bool ret = false;
		if (!model)
		{
		...
		if (modelShape->IsDrawDataPrepared())
		{
			...
			if (srcBodys)
			{
				...
				if (tmpBodys.size() > 0)
				{
					...
					{
						model->AddBody(tmpBodys[i]);
						model->SetProtoTypeId(model->GetProtoTypeId() + 1);
					}
				}
			}
			...
			for (int i = 0; i < childModels.size(); i++)
			{
				...
				MergeModelFaces(child, MergeFlg, iEliminateFlg, false);
			}
		}
	...
	bool Merge::HaveInteraction(vector<Body*>* bodys, Body* targetBody)
	{
		if (!bodys || !targetBody)
		{
			return false;
		...
		bool bSameColor = false;
		vector<Face*>& targetFaces = targetBody->GetFaces();
		for (size_t i = 0; i < targetFaces.size() && !bSameColor; i++)
		{
			Face* targetFace = targetFaces[i];
			for (size_t j = 0; j < bodys->size() && !bSameColor; j++)
			{
				Body* body = bodys->at(j);
				vector<Face*>& Faces = body->GetFaces();
				for (size_t m = 0; m < Faces.size(); m++)
				{
					Face* face = Faces[m];
					if (Merge::CompareColor(face->GetColor(), targetFace->GetColor()))
					{
						bSameColor = true;
						break;
					}
		...
		VertexSet* targetVertex = targetBody->GetData();
		vector<Vector3>* targetPnts = targetVertex->GetPositionArray();

		for (size_t i = 0; i < targetPnts->size(); i++)
		{
			Vector3 targetPnt = targetPnts->at(i);
			for (size_t j = 0; j < bodys->size(); j++)
			{
				Body* body = bodys->at(j);
				VertexSet* vertex = body->GetData();
				vector<Vector3>* pnts = vertex->GetPositionArray();

				if (find(pnts->begin(), pnts->end(), targetPnt) != pnts->end())
				{
					return true;
				}
			}
