...
void DraggerTransformCallback::UpdteAixsLineTip(const MotionCommand& command)
{
	...
	//更新X轴
	if (refDragger->GetDraggerName() == L"XAxis")
	{
		...
		if (stage == MotionCommand::START)
		{
			...
			handlerGroup->AddSVLTool(xAxisLineModel, XAXISLINENAME);
		}
		else if (stage == MotionCommand::MOVE)
		{
			//在轴线上面添加文字
			Model* xAxisLineX = handlerGroup->GetSVLTool(XAXISLIE_X);
			if (xAxisLineX)
			{
				handlerGroup->RemoveSVLTool(XAXISLIE_X);
			}
            ...
            Vector3 temp1 = (textPos + m_startPos) / 2;
			textImageX->SetImageSize(temp1, tempSize);

			handlerGroup->AddSVLTool(textImageX, XAXISLIE_X);
		}
		else if (stage == MotionCommand::FINISH)
		{
			//删除添加的模型
			Model* xAxisLine = handlerGroup->GetSVLTool(XAXISLINENAME);
			if (xAxisLine)
			{
				handlerGroup->RemoveSVLTool(XAXISLINENAME);
			}
			...
			/*Model* xAxisLineNX = m_dragger->GetDrawModel()->GetModelByName(XAXISLIE_NX);
			if (xAxisLineNX)
			{
				refDragger->GetDrawModel()->RemoveSubModel(xAxisLineNX);
			}*/
		}
	}
}
