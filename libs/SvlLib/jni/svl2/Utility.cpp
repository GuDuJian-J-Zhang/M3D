...
std::string ColorToString(const int& red, const int& green, const int& blue,
		const int& alpha) {
	std::stringstream ss;
	ss << red << ", ";
	ss << green << ", ";
...
wstring WCHARStrToWString(unsigned short * srcStr) {
	...
	int errCode = 0;
	int ret = 0;
	ret = UCNVConvert(UTF_32LE, UTF_16LE, tarBuf, tarBufLen,
