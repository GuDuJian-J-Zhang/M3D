...
public:
	...
	BOOL DesEncryptFile(CString filename,const char* password);
	BOOL DesDecryptFile(CString filename,const char* password);
	BOOL DesBytesEncrypt(char *Out, long OutSize,char *In, long InSize,const char *Key);
	BOOL DesBytesDecrypt(char *Out, long OutSize,char *In, long InSize,const char *Key);
	VOID  DesEncryptString(CString &input,CString &output, const char key[9]);
	VOID  DesDecryptString(string &input, string &output,const char key[9]);
	CString CStringToHex(char input[],int length);
	VOID HexToCString(char  output[],string& hexStr);
private:
