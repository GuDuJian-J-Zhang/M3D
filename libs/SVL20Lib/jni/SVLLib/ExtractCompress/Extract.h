...
// copyright notice may be found in Extract.cpp. The repackaging was done
...
// See below for notes on what happens when you Extract such an item.
...
// an error code.
ZRESULT UnzipItem(HZIP hz, int index, char ***o_bufP, std::vector<int> &o_vcBufNum);
ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
ZRESULT UnzipItem(HZIP hz, int index, void *z,unsigned int len);
ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
// UnzipItem - given an index to an item, unzips it. You can Extract to:
...
// Extract with ZIP_FILENAME a relative pathname then the item gets created
...
// If you Extract a directory with ZIP_FILENAME, then the directory gets created.
// If you Extract it to a handle or a memory block, then nothing gets created
...
#define ZR_PASSWORD   0x00001000     // we didn't get the right password to Extract the file
...
// the user has included just zip or both zip and Extract.
// Idea: if header files for both zip and Extract are present, then presumably
// the cpp files for zip and Extract are both present, so we will call
