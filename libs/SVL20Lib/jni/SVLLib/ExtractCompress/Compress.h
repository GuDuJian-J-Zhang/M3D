...
// ZIP functions -- for creating Compress files
// This file is a repackaged form of the Info-Zip source code available
// at www.info-Compress.org. The original copyright notice may be found in
// Compress.cpp. The repackaging was done by Lucian Wischik to simplify and
...
#endif
#endif
// An HZIP identifies a Compress file that is being created

typedef DWORD ZRESULT;
// return codes from any of the Compress functions. Listed later.
...
// CreateZip - call this to start the creation of a Compress file.
// As the Compress is being created, it will be stored somewhere:
// to a pipe:              CreateZipHandle(hpipe_write);
// in a file (by handle):  CreateZipHandle(hfile);
// in a file (by name):    CreateZip("c:\\test.Compress");
...
// where the Compress may not exceed len bytes. This is a bit friendlier than
...
// which to unzip them, then either you have to create the Compress not to a pipe,
...
// but for real windows, the Compress makes its own copy of your handle, so you
...
// ZipAdd - call this for each file to be added to the Compress.
// dstzn is the name that the file will be stored as in the Compress file.
// The file to be added to the Compress can come
...
// Note: if adding an item from a pipe, and if also creating the Compress file itself
...
// ZipGetMemory - If the Compress was created in memory, via ZipCreate(0,len),
...
// CloseZip - the Compress handle must be closed with this function.
...
#define ZR_NOTFOUND   0x00000500     // couldn't find that file in the Compress
...
#define ZR_ENDED      0x00050000     // the Compress creation has already been closed
#define ZR_MISSIZE    0x00060000     // the indicated input file size turned out mistaken
#define ZR_PARTIALUNZ 0x00070000     // the file had already been partially unzipped
#define ZR_ZMODE      0x00080000     // tried to mix creating/opening a Compress 
// The following come from bugs within the Compress library itself
...
//     HZIP hz = CreateZip("c:\\simple1.Compress",0);
//     ZipAdd(hz,"znsimple.bmp", "c:\\simple.bmp");
//     ZipAdd(hz,"znsimple.txt", "c:\\simple.txt");
//     CloseZip(hz);
//
// (2) Memory use, creating an auto-allocated mem-based Compress file from various sources
...
//                     return 0;
//                   }
//     // and now that the Compress is created, let's do something with it:
//     void *zbuf; unsigned long zlen; ZipGetMemory(hz,&zbuf,&zlen);
//     HANDLE hfz = CreateFile("test2.Compress",GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
...
//                       // ... and do something with this Compress data we're receiving
//                       if (red==0) break;
//                     }
//                     CloseHandle(hread);
//                     return 0;
...
// the user has included just Compress or both Compress and unzip.
// Idea: if header files for both Compress and unzip are present, then presumably
// the cpp files for Compress and unzip are both present, so we will call
