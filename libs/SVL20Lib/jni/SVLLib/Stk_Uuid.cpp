...
#pragma warning(pop)		/* C4996 */
	...
	srand((dword_t) ((((dword_t)timebuf.time&0xFFFF)+(dword_t)timebuf.millitm)^(dword_t)timebuf.millitm));
#endif
#ifdef __MOBILE__
	struct timeb timebuffer;
	ftime(&timebuffer);
	srand((dword_t) ((((dword_t)timebuffer.time&0xFFFF)+(dword_t)timebuffer.millitm)^(dword_t)timebuffer.millitm));
#endif
	return rand();
};


bool stk::uuid::is_uuid_string(const char *uuid) 
{
 ...
 */
void stk::uuid::uuid_create(uuid_t* u) 
{
	static bool		initialized = false;
	static int64	timestamp;
	static dword_t	advance;
	static uchar	clockseq;
	static uchar	node_high;
	static dword_t	node_low;
 ...
 * for all the threads running within a process.
 */
char * stk::uuid::uuid_create_string(void) 
{
	uuid_t  u;
	uuid_create(&u);
...
char * stk::uuid::uuid_to_string(const uuid_t*  u)
{
...
//                 u->data3,
//                 a, b, c);
	sprintf(uuid_str, "%08lx-%04x-%04x-%08lx", 
		u->data1,
		u->data2,
		u->data3,
		c);
...
void stk::uuid::uuid_create_external(const char *external, uuid_t* u) 
{
	stk::MD5::MD5_CTX md5;
	...
	stk::MD5::MD5_init(&md5);
	stk::MD5::MD5_update(&md5, namespace_uuid, sizeof(namespace_uuid));
	stk::MD5::MD5_update(&md5, (unsigned char *) external, (unsigned int) strlen(external));
	stk::MD5::MD5_finish(uuid, &md5);
 ...
 **/
void stk::uuid::uuid_to_timestamp(const uuid_t* u, timestamp_t* t)
{
    int64   time, t2, t3;
    struct  tm*  p;
    assert(u);
